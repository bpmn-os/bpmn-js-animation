...
Refusal is by **rule**: `ModeRules` denies the rules diagram-js asks before it offers a gesture, so what is forbidden is not offered rather than offered and ignored — an entry is not drawn, handles are not shown, a drag does not engage. An exception lifts that refusal where it names the operation and applies to every element the rule concerns, and the rules below then have their say, so a note may be moved because the host permits it and still not be dropped where bpmn-js forbids. The context pad opens only where an entry is kept and shows the kept entries alone; the Delete key removes what the rules permit and nothing else; programmatic modelling is untouched, a host calling `modeling.moveShape` doing so on purpose; and an element a move or a resize is permitted on keeps its selection outline, which the simulation view otherwise hides, along with the resize handles, drawn in the canvas's own layer and shown while everything selected may be resized. Only the outermost call is judged, since how an operation decomposes into others is no business of the host permitting it. `mode.allows(operation, element)` and `mode.entriesFor(element)` answer the same questions to a host driving the modeller itself.
