...
opinionated *tool* on top of the [`animation`](animation.md) vocabulary — the mirror of the
...
Add the **`AnimatorModule`** (`primitives` + `animation` + `animator`) — or the full default export,
...
`{ action, ...fields }`, where `action` is a [`animation`](animation.md) method name and the
...
the `animation` verbs it drives, so it can capture a run as a log in this exact format.

| Method (on `simulator`) | Description |
| --- | --- |
| `startRecording()` | Begin recording — every subsequent `animation` token-flow call is appended to the log. Resets any prior recording. |
...
Recording captures any call on the shared `animation` instance, so it works whether the simulator's
