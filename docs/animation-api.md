...
| `moveToken(fromNode, label, toNode, selector?, state?)` | **Teleport** a token to another node, **preserving its identity** (same object — color, selection, instance, children carried across), re-keying it from `fromNode` to `toNode` and re-rendering both. An instantaneous relocation with no flow travel — the **link-event** primitive (a link throw → its matching catch). `state` merges into the landing rest state. Returns the token. |
