 ...
 */
Tokens.prototype.setAnimationDuration = function(duration) {
  this._animation.setAnimationDuration(duration);
};
 ...
 */
Tokens.prototype.setFilter = function(predicate) {
  this._filter = typeof predicate === 'function' ? predicate : null;

  // re-render resting badges
  for (const node of Array.from(this._nodeTokens.keys())) {
    this._renderNode(node);
  }

  // update in-flight (moving) tokens
  for (const [ token, animation ] of this._activeAnimations) {
...
/**
 * Play the element's own symbol as a one-off animation, triggered by the host.
 * Throwing elements (throw/end events, send task) emit the symbol diagonally to
 * the upper-right and fade it out; catching elements (start/catch/boundary
 * events, receive task) draw it in diagonally from the upper-left and fade it
 * in. Native symbol color; shared animation duration. No-op if the element has
 * no symbol.
 *
 * @param {string} node element id
 * @return {Promise<void>} resolves when the effect ends
 */
Tokens.prototype.animateSymbol = function(node) {
  ...
  const symbols = symbolNodes(gfx, element);
  ...
  // outer group positions the (element-local) symbol over the element;
  ...
  const cls = this._isThrowing(element) ? 'bts-symbol-emit' : 'bts-symbol-receive';
  svgAttr(inner, 'class', `bts-symbol ${cls}`);
  inner.style.animationDuration = this._animation.getAnimationDuration() + 'ms';
  ...
  return new Promise(resolve => {
    ...
    setTimeout(finish, this._animation.getAnimationDuration() + 50);
  });
};
...
Tokens.prototype._isVisible = function(token) {
  return !this._filter || !!this._filter(token);
};

Tokens.prototype._planeLayer = function(element) {
...
Tokens.prototype._isThrowing = function(element) {
  // throwing = send task + throw/end events; everything else animates as catching
  // (receive task and all other typed tasks, start/catch/boundary events)
...
/**
 * The element's symbol/marker geometry to clone for `animateSymbol`: any child
 * shape that is NOT the full-size body/outline (the event boundary ring, task
 * rect, gateway diamond, …). Identified by **size, not tag** — so a symbol drawn
 ...
 */
function symbolNodes(gfx, element) {
  if (!gfx) {
    return [];
  }
  ...
  return Array.from(shapes).filter(el => {
    ...
    // drop the body/outline (fills ~the whole element); keep smaller markers/symbols
