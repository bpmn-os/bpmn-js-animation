...
export default function SimulationAPI(eventBus, animation, elementRegistry) {
  ...
  this._recording = null; // when an array, each call to a RECORDED_METHOD is logged for replay
  this._installRecorder();
...
// The **BPMN events** captured by the recorder — the semantic token-flow operations an execution
// engine emits, logged as `[method, ...args]` and replayed by `simulation[method](...args)`. The
// view-navigation methods (`moveToFront` / `moveToBack` / `scrollStack`) and the focus settings
// (`autoFocus` / `setFocusContext`) are deliberately **not** recorded — a replay derives which
// instance to show from the events themselves (run it with `autoFocus` on), so the log is a clean
// event log, not a transcript of how a user scrolled.
...
function describeEvent(method, args) {
...
function startsImmediately(event) {
  return event.action === 'advanceToken' && event.sequenceFlow != null;
}

// Wrap each recorded method (per instance) so a successful call appends a flat event object to the
// recording.
SimulationAPI.prototype._installRecorder = function() {
  for (const method of RECORDED_METHODS) {
    const original = SimulationAPI.prototype[method];
    this[method] = (...args) => {
...
/**
 * Begin recording the **BPMN events** ({@link RECORDED_METHODS}) as they happen — e.g. while a user
 * drives the interactive `simulator`. Each is appended to the log as `[method, ...args]`. View
 * navigation and focus settings are not recorded. Replay with {@link replay} (run it with
 * `autoFocus` on so each event reveals its instance); download `getRecording()` as JSON.
 */
SimulationAPI.prototype.startRecording = function() {
  this._recording = [];
};

/** Stop appending to the recording and return it (the log is kept; `startRecording` resets it). */
SimulationAPI.prototype.stopRecording = function() {
  const log = this._recording || [];
  this._recording = null;
  return log;
};

/** The recording so far — an array of `[method, ...args]` entries (a copy). */
SimulationAPI.prototype.getRecording = function() {
  return (this._recording || []).slice();
};

/**
 * Replay a recorded `log` (an array of flat `{ action, ...fields }` events) against this service, in
 * order. Most events are an **arrival/settle** and play **serially** — each waits for every previously
 * started animation to complete, then plays at the animation speed (so the run paces itself, exactly
 * like driving it live). The **departures/placements** — a token travelling out along a flow, a
 * `forkToken`, a `createToken` — start **immediately** instead (see {@link startsImmediately}), so the
 * branches of a diverging gateway and concurrently-spawned instances leave **together** rather than
 * one-after-another. The next arrival/settle event drains them, so concurrency only appears where those
 * immediate events are adjacent. Pass `gate` (awaited before each event) to pause/abort; not recorded.
 *
 * @param {Array<object>} log
 * @param {{ gate?: () => (void | Promise<void>) }} [options]
 * @return {Promise<void>}
 */
SimulationAPI.prototype.replay = async function(log, { gate } = {}) {
  ...
  for (const event of log) {
    ...
    // Departure/placement: start now, concurrently with whatever is still animating. No reveal-before —
    ...
    if (startsImmediately(event)) {
      outstanding.push(Promise.resolve(eventCall(this, event)));
      continue;
    }
    ...
    if (this._autoFocus) {
      ...
      const token = node != null && this.getToken(node, event.label);
      if (token) {
...
SimulationAPI.prototype.consumeToken = async function(args) {
  const { label, gesture, sequenceFlow } = args;
  const node = this._canonicalId(args.node); // accept a process id (→ its participant shape)
 ...
 * **front** — the on-screen one. Pass-through to the `animation` service. A host driving several
 ...
 * @return {Promise<void>}
 */
SimulationAPI.prototype.moveToFront = function(node, label) {
  return this._animation.moveToFront(this._canonicalId(node), label);
};

/**
 * Send a stacked node's instance (keyed by **label**) to the **back** — pass-through to the
 * `animation` service. Animates only when the shown (front) instance changes.
 *
 * @param {string} node
 * @param {string} label the instance's label
 * @return {Promise<void>}
 */
SimulationAPI.prototype.moveToBack = function(node, label) {
  return this._animation.moveToBack(this._canonicalId(node), label);
};

/**
 * Scroll a stacked node to the next (`'forward'`) / previous (`'backward'`) instance — thin sugar
 * over `moveToFront`/`moveToBack`, pass-through to the `animation` service.
 *
 * @param {string} node
 * @param {'forward'|'backward'} [direction='forward']
 * @return {Promise<void>}
 */
SimulationAPI.prototype.scrollStack = function(node, direction = 'forward') {
  return this._animation.scrollStack(this._canonicalId(node), direction);
};
...
SimulationAPI.prototype._tokenOnFlow = function(node, sequenceFlow) {
  ...
  for (const entry of this._tokenMap.values()) {
    if (entry.node === node && entry.sequenceFlow === sequenceFlow) {
      if (this._animation._isVisible(entry.token)) {
        return entry;
      }
 ...
 */
SimulationAPI.prototype._canonicalId = function(id) {
