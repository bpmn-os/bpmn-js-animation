 ...
 * the token's real selector (rest flow + instance `stackIndices`) so the low-level lookup finds
 * it — callers address it by readable `(node, label[, sequenceFlow])`. No-op if absent. `→ Promise`.
 */
SimulationAPI.prototype.playTokenEffect = function(node, label, effect, selector) {
  ...
  return this._animation.playTokenEffect(node, label, effect,
    { sequenceFlow: entry.sequenceFlow || undefined, stackIndices: entry.stackIndices });
};
