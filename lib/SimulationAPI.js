 ...
 *   `node`: process/participant, a start event, or a boundary event; `label`: the instance id.
...
SimulationAPI.prototype.createToken = function(args) {
  ...
  throw new Error(`createToken: "${node}" is not a process/participant, a start event, or a boundary event`);
};
...
SimulationAPI.prototype.consumeToken = async function(args) {
  ...
  if (isAny(element, [ 'bpmn:Process', 'bpmn:Participant' ])) {
    ...
    this._animation.setStacks(node, this._animation.getStacks(node).filter(k => k !== key), {});
  }
