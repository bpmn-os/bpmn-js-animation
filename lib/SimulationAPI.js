...
SimulationAPI.prototype.consumeToken = async function(args) {
  ...
  // stack-decrement: consuming a process/participant instance (its root sits at the host)
  // shrinks the host's stack — consuming the last instance removes its box. MI activities and
  // event sub-processes decrement too; that lands with their fan-out / listener support.
  if (isAny(element, [ 'bpmn:Process', 'bpmn:Participant' ])) {
    const size = this._animation.getStackSize(node);
    if (size >= 1) {
      this._animation.setStackSize(node, size - 1);
    }
  }
