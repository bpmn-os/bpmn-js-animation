...
SimulationAPI.prototype.consumeToken = async function(args) {
  ...
  // synchronously here; the flip-fade ghosts (when `effects`) play independently afterwards
  const subtree = this._tearDown(target, effects);
...
SimulationAPI.prototype._tearDown = function(token, effects) {
  ...
  // (one synchronous loop), while the model identities all drop now.
  for (const t of subtree) {
    const e = this._tokenMap.get(t);
    if (e) {
      this._animation.removeToken(e.node, e.label, { sequenceFlow: e.sequenceFlow || undefined, stackIndices: e.stackIndices }, effects);
