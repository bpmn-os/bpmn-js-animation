 ...
 *    inheriting its color, at `ready`. Rejected once the first sub has entered (the spawn window
...
SimulationAPI.prototype.createToken = function(args) {
  ...
  if (is(element, 'bpmn:BoundaryEvent')) {
    return this._createBoundaryToken(node, label, element, bounce);
  }
  if (classify(element).multiInstance) {
    return this._createMultiInstanceToken(node, label, element, bounce);
  }
...
SimulationAPI.prototype._createBoundaryToken = function(node, label, element, bounce) {
...
// (in the outer-instance context), inheriting its color, at `ready`. The spawn window stays open
// until the first sub enters (`advanceToken` parks the parent); spawning after that is rejected.
SimulationAPI.prototype._createMultiInstanceToken = function(node, label, element, bounce) {
  ...
  this._register(node, label, token, stackIndices, parent.token, Position.READY);
...
SimulationAPI.prototype.advanceToken = async function(args) {
  ...
  // MI: when a sub-instance starts running (leaves `ready`), park the outer thread token — it
  // can no longer spawn instances (the spawn window closes). Idempotent once parked.
  if (position !== Position.READY && entry.stackIndices[node] != null && classify(element).multiInstance) {
...
SimulationAPI.prototype.consumeToken = async function(args) {
  ...
  // stack-decrement: if the target was a **stacked instance** of `node`, drop *its* key (not just
  // the count) — one rule covering process/participant roots AND MI sub-instances — so survivors
  // keep rendering and the last one clears the host. Done in the target's outer-instance context.
  const key = target.stackIndices && target.stackIndices[node];
  if (key != null) {
    const keys = this._animation.getStacks(node); // node's stack in its current context
    if (keys.includes(key)) {
      this._animation.setStacks(node, keys.filter(k => k !== key));
      ...
      if (keys.length === 1 && classify(element).multiInstance) {
        this._unparkMIParent(node, element, parentToken);
      }
    }
  }
...
// MI fan-in: reveal the parked outer thread token and move it onto the activity's (single)
// outgoing flow (rekey incoming→outgoing) — the host then `advanceToken`s it onward.
SimulationAPI.prototype._unparkMIParent = function(node, element, parentToken) {
  ...
  this._animation.setState(pe.node, pe.label, { hidden: false, sequenceFlow: outFlow.id },
    { sequenceFlow: pe.sequenceFlow || undefined, stackIndices: pe.stackIndices });
