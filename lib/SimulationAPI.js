...
// Advance along a flow: move the token onto `sequenceFlow` (a connected flow) and travel it
// to the far node, where it rests on the same flow (outgoing → forward, incoming → reverse —
// direction is sendToken's call). Re-keys to the far node; identity + children are carried.
