...
export default function createTokenEntry(token, options = {}) {
  ...
  const entry = expandable
    ? createCollapsibleEntry({ label: summary, open: !!open, toggleOn: 'caret' })
    : createSimpleEntry({ label: summary });
  ...
  return { element: entry.element, update, contentEl: entry.contentEl || null, token: () => token };
}
