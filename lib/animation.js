...
Animation.prototype.createToken = function(args) {
  ...
  if (isAny(element, [ 'bpmn:Process', 'bpmn:Participant' ])) {
    ...
    token = this._createCatchEventToken(node, label, element, animate);
...
Animation.prototype._createCatchEventToken = function(node, label, element, animate) {
  const scope = this._scopeOf(element);
  if (!scope) {
    throw new Error(`createToken: link catch event "${node}" has no enclosing scope`);
  }
  const parent = this.getEntry(scope.id, label);
  if (!parent) {
    throw new Error(`createToken: no token <${label}> at scope <${scope.id}>`);
...
Animation.prototype._createMultiInstanceToken = function(node, label, element, animate) {
  const parent = this._miParentFor(element, label);
  if (!parent) {
    // no outer parent to spawn from
    if ((element.incoming || []).length) {
      // normal MI: the outer thread hasn't arrived on the incoming flow yet
      throw new Error(`createToken: no token resting on <${node}>'s incoming flow to spawn an instance from`);
    }
    // ad-hoc child with **no incoming flow**: this is the outer/main token itself — seed it at ARRIVAL
    return this._createMIMainToken(node, label, element, animate);
  }
...
// The outer parent a sub of `label` spawns from: the token on the (single) **incoming flow** (normal
// MI), or — for an **ad-hoc child with no incoming flow** — the **main token resting at the node** (the
// sub label without its trailing `#k` slot). Returns the parent **entry**, or `null` when `label` is the
// main token itself (no parent to spawn from → the caller seeds the main via `_createMIMainToken`).
Animation.prototype._miParentFor = function(element, label) {
  const inFlow = (element.incoming || [])[0];
  if (inFlow) {
    return this._tokenOnFlow(element.id, inFlow.id);
  }
  const i = String(label).lastIndexOf('#');
  if (i < 0) {
    return null; // no slot → this is the main token, not a sub
  }
  return this.getEntry(element.id, String(label).slice(0, i)) || null;
};

// MI activity used as an **ad-hoc child with no incoming flow**: create the outer/main token — a child
...
Animation.prototype._createMIMainToken = function(node, label, element, animate) {
  const scope = this._scopeOf(element);
  if (!scope) {
    throw new Error(`createToken: MI activity "${node}" has no enclosing scope`);
  }
  const parent = this.getEntry(scope.id, label);
  if (!parent) {
    throw new Error(`createToken: no token <${label}> at scope <${scope.id}>`);
