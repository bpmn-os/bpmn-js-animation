...
Animation.prototype._createEventSubProcessToken = function(node, label, evtsp, animate, parent) {
  ...
  // No auto-focus reveal: an event-sub waiter is a new but idle firing instance. When a non-interrupting
  // event-sub fires, the firing departs (the active instance) and a fresh waiter is armed here; revealing
  // the waiter would scroll the just-triggered firing out of view and back. Focus follows the firing, not
  // the new waiter, so return the token without `_touched`.
  return token;
};
...
Animation.prototype._createMIMainToken = function(node, label, element, animate, parent) {
  const stackIndices = { ...parent.stackIndices };
  const state = { position: positionFor(Position.ARRIVAL), animate };
  const token = this._primitives.createToken(node, label, parent.token.color, state, stackIndices);
