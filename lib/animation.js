...
Animation.prototype._travelFlow = async function(node, label, sequenceFlow, element, onDeparted) {
  ...
  // a **catch** or typed **start** event flies its symbol IN before the token departs (this is the
  // trigger) — awaited, so the token leaves only once the icon has landed. A no-op for a plain node.
  const ev = classify(element).event;
  if (ev === 'catch' || ev === 'start') {
