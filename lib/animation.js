...
/** Whether auto-focus is on. Lets a replay driver mirror the setting (reveal an instance *before*
 *  animating its step, as the Animator does) instead of relying only on the post-op reveal. */
Animation.prototype.isAutoFocus = function() {
...
Animation.prototype._reset = function() {
  ...
  // drives getDistinctColor for fresh identities. A random start (not 0) so each run begins at a
  // different palette color — otherwise every fresh viewer/clear restarts the cycle and the first
  // instance is always the same color (the palette itself is fixed per page load). getDistinctColor
  // wraps the index, so any non-negative start works.
  this._colorIndex = Math.floor(Math.random() * 1e6);
...
/**
 * Await (and clear) a token's pending entrance gesture — the `createToken` fade-in + flip. Lets a replay
 * driver show the flip on creation before the token's next step (the analogue of a depart awaiting it, so
 * the flip is never cut short). Resolves immediately when there is no pending entrance.
 */
Animation.prototype.whenEntered = function(node, label) {
  return this._awaitEntering(this.getToken(node, label));
};
 ...
 *    cascade: it's deleted when the activity departs (its children are shed) or is consumed. An interrupting
...
Animation.prototype.createToken = function(args) {
  const { label, animate, parentNode, parentLabel } = args;
  ...
  // Every token but a process/participant instance root is a child of an explicitly-named parent — the
  // caller owns the token hierarchy; the library infers nothing. The parent is referenced by
  // (parentNode, parentLabel) so it is serialisable in an execution log.
  const root = isAny(element, [ 'bpmn:Process', 'bpmn:Participant' ]);
  let parent = null;
  if (!root) {
    if (parentNode == null || parentLabel == null) {
      throw new Error(`createToken: <${node}> needs a parent (parentNode/parentLabel); only a process/participant instance root may omit it`);
    }
    parent = this.getEntry(this._canonicalId(parentNode), parentLabel);
    if (!parent) {
      throw new Error(`createToken: parent token <${parentLabel}> at <${parentNode}> not found`);
    }
  }
  ...
  if (root) {
    ...
    token = this._createStartEventToken(node, label, element, animate, parent);
  } else if (is(element, 'bpmn:IntermediateCatchEvent') && classify(element).link !== undefined) {
    token = this._createCatchEventToken(node, label, element, animate, parent);
  } else if (is(element, 'bpmn:BoundaryEvent')) {
    token = this._createBoundaryToken(node, label, element, animate, parent);
  } else if (classify(element).multiInstance) {
    token = this._createMultiInstanceToken(node, label, element, animate, parent);
  } else if (is(element, 'bpmn:Activity')) {
    token = this._createActivityToken(node, label, element, animate, parent);
  } else {
    throw new Error(`createToken: "${node}" is not a process/participant, a start/boundary event, a link catch event, an activity, or an MI activity`);
  }
  ...
  //
  // Deferred behind any reveal arc THIS creation just queued: `_touched` → `_bringToFront` (auto-focus)
  // scrolls the new instance to the front and accumulates that arc into `_focusing`. Chaining the flip
  // after it plays the fade-in+flip once the stack has finished scrolling — so the flip is seen at the
  // front instead of before/behind the scroll. With no reveal in flight `_focusing` is already resolved,
  // so the flip still plays immediately (unchanged for non-stacked nodes / auto-focus off).
  if (this._primitives.getAnimationDuration() > 0) {
    const fx = { sequenceFlow: token.state.sequenceFlow || undefined, stackIndices: token.stackIndices };
    // a token created hidden (so it wouldn't ride the reveal scroll — see _createMultiInstanceToken) is
    // unhidden here, at the front slot, just before its flip — so it appears only AFTER the scroll.
    const hiddenForReveal = !!token.state.hidden;
    const enter = () => {
      if (hiddenForReveal) {
        this._primitives.setState(node, label, { hidden: false }, fx);
      }
      return this._primitives.playTokenEffects(node, label, ENTRANCE, fx);
    };
    this._entering.set(token, this._focusing.then(enter, enter));
  }

  return token;
};

// Whether a newly created dot should be born **hidden**: when auto-focus is on it will be scrolled to
// the front by `_touched` (`moveToFront`), and a hidden dot is excluded from that scroll's snapshot
// (`_appendTokenDot`), so the stack scrolls with the new dot ABSENT — then `createToken`'s deferred
// entrance unhides + flips it in once the scroll settles. Hence the sequence is scroll → dot appears →
// flip, never dot-rides-scroll. With auto-focus off / animation off the dot is born visible as before.
Animation.prototype._newDotHidden = function() {
  return this._autoFocus && this._primitives.getAnimationDuration() > 0;
};
...
Animation.prototype._createActivityToken = function(node, label, element, animate, parent) {
  const stackIndices = { ...parent.stackIndices };
  const state = { position: positionFor(Position.ENTRY), animate, hidden: this._newDotHidden() };
...
// Link catch event: a child of the enclosing scope's token, same label and color, at `center`. A link
// catch has no incoming flow, so this is how a token reaches it. The simulator and a replay create the
// token here and consume the matching link throw; inheriting the scope's color keeps the instance's
// color across the link.
Animation.prototype._createCatchEventToken = function(node, label, element, animate, parent) {
  const stackIndices = { ...parent.stackIndices };
  const state = { position: positionFor(Position.CENTER), animate, hidden: this._newDotHidden() };
  const token = this._primitives.createToken(node, label, parent.token.color, state, stackIndices);
...
Animation.prototype._createStartEventToken = function(node, label, element, animate, parent) {
  const scope = this._scopeOf(element);
  if (scope && classify(scope).eventSubProcess) {
    return this._createEventSubProcessToken(node, label, scope, animate, parent);
  }
...
Animation.prototype._createEventSubProcessToken = function(node, label, evtsp, animate, parent) {
  const stackIndices = { ...parent.stackIndices, [evtsp.id]: label };
  // stack the firing in the context of the instance it belongs to (parent's ancestor keys), NOT the
  // current front — else with several enclosing instances (e.g. many machine processes) the firing
  // lands in the wrong bucket and its cleanup on consume can never find it.
  const ctx = parent.stackIndices;
  // is this the FIRST firing of this enclosing instance's event-sub? (its event-sub node not yet stacked
  // in this context). The first firing reveals the instance; later ones must not (see below).
  const firstFiring = this._primitives.getStacks(evtsp.id, ctx).length === 0;
  this._primitives.setStacks(evtsp.id, [ ...this._primitives.getStacks(evtsp.id, ctx), label ], ctx);

  // Born hidden (like every other new dot) so it does not ride a stack scroll happening around it — the
  // deferred entrance in `createToken` unhides + flips it in once `_focusing` settles.
  const state = { position: positionFor(Position.CENTER), animate, hidden: this._newDotHidden() };
  const token = this._primitives.createToken(node, label, parent.token.color, state, stackIndices);

  this._register(node, label, token, stackIndices, parent.token, Position.CENTER);

  // Reveal only on the FIRST firing: `_touched` brings the firing's **enclosing instance** to the front
  // (e.g. its machine process), so the firing is created once that scroll has completed rather than riding
  // it — and since the event-sub node holds just this one firing, its own stack does NOT scroll. A LATER
  // firing must skip the reveal: the enclosing instance is already at front, and `_touched` would instead
  // scroll the event-sub's own (now multi-firing) stack, yanking the currently-active firing out of view.
  return firstFiring ? this._touched(token) : token;
};
...
Animation.prototype._createBoundaryToken = function(node, label, element, animate, parent) {
  const stackIndices = { ...parent.stackIndices };
  const state = { position: positionFor(Position.CENTER), animate, hidden: this._newDotHidden() };
  const token = this._primitives.createToken(node, label, parent.token.color, state, stackIndices);
...
Animation.prototype._createMultiInstanceToken = function(node, label, element, animate, parent) {
  // `parent` (resolved by createToken) distinguishes main from sub with no naming convention: a parent
  // AT THIS node is the main token → create a stacked sub; a parent at the ENCLOSING SCOPE means the
  // caller is creating the MI main token itself.
  if (parent.node !== node) {
    return this._createMIMainToken(node, label, element, animate, parent);
  }
  ...
  // stack the sub in the context of the MI main's own instance (not the current front) — consistent
  // with cleanup on consume, so it works even under several enclosing instances.
  const miCtx = parent.stackIndices;
  this._primitives.setStacks(node, [ ...this._primitives.getStacks(node, miCtx), label ], miCtx);
...
// MI activity: create the outer/main token — a child
// of the enclosing scope (inheriting its color), resting **small at the ARRIVAL point** (left-center).
// It is the spawn point: `_phaseOf` reads ARRIVAL as `arrived`, so a double-click spawns a sub, exactly
// like the inflow parent in the normal case. It never enters; subs stack at the node.
Animation.prototype._createMIMainToken = function(node, label, element, animate, parent) {
  const stackIndices = { ...parent.stackIndices };
  const state = { position: positionFor(Position.ARRIVAL), animate, hidden: this._newDotHidden() };
  const token = this._primitives.createToken(node, label, parent.token.color, state, stackIndices);

  this._register(node, label, token, stackIndices, parent.token, Position.ARRIVAL);
...
Animation.prototype.advanceToken = async function(args) {
  ...
  // a **boundary** firing along its outflow is special: the boundary's dot does not travel as-is. It
  // becomes a fresh dot on the outflow (a child of the enclosing scope), and when interrupting the host
  // activity is cancelled. See `_fireBoundary`.
  if (sequenceFlow && classify(element).event === 'boundary') {
    return this._fireBoundary(node, label, sequenceFlow, element);
  }

  // along a flow → travel to the far node (rests on the flow there). `onDeparted` (if given) fires the
  // instant the token has left the source node (after the source re-renders).
  if (sequenceFlow) {
    return this._travelFlow(node, label, sequenceFlow, element, args.onDeparted);
  }

  // into a center node → anchor at the symbol center. A **throw** or **end** event flies its symbol OUT
  // as it arrives here (a link throw too — it's a throw event), fire-and-forget. Catch events also anchor
  // here but fly their symbol on depart instead (see `_travelFlow`).
  if (anchorsAtCenter(element)) {
    const token = await this._anchorAtCenter(node, label, animate);
    const ev = classify(element).event;
    if (ev === 'throw' || ev === 'end') {
      this._primitives.throwIcon(node, label, { stackIndices: token.stackIndices });
    }
    return token;
  }
  ...
  // send / receive **task** message icon: a send task flings its symbol OUT as it starts working
  // (→ busy, fire-and-forget); a receive task waits for its symbol to fly IN (→ completion, awaited)
  // before it completes. A no-op for a node with no message symbol.
  const task = classify(element).task;
  if (task === 'send' && position === Position.BUSY) {
    this._primitives.throwIcon(node, label, selector);
  }
  if (task === 'receive' && position === Position.COMPLETION) {
    await this._primitives.catchIcon(node, label, selector);
  }
...
Animation.prototype._travelFlow = async function(node, label, sequenceFlow, element, onDeparted) {
  ...
  // a **catch** event, or a **typed** start event (message / timer / signal — one with an event
  // definition), flies its symbol IN before the token departs (the trigger), awaited so the token
  // leaves only once the icon has landed. A plain (none) start event has no symbol, so it skips this
  // entirely — no icon and no wait, it just travels.
  const ev = classify(element).event;
  const typedStart = ev === 'start' && (element.businessObject.eventDefinitions || []).length > 0;
  if (ev === 'catch' || typedStart) {
    await this._primitives.catchIcon(node, label, { sequenceFlow: entry.sequenceFlow || undefined, stackIndices });
  }

  // move onto the flow (clears any cue). Skip when the token is already resting on this exact flow —
  // e.g. a forked branch that `forkToken` already placed on its outflow — so a redundant setState
  // doesn't re-render the node.
  ...
  // an **interrupting event sub-process** firing cancels the rest of its enclosing scope as it departs.
  // The firing is now on the flow (a counting token, not a waiter at its start), so the scope survives.
  // Done here, internally, so it isn't a separate recorded action that replay would serialise after the
  // travel — the same shape as a boundary's host cancel.
  if (ev === 'start') {
    const evtsp = this._scopeOf(element);
    if (evtsp && classify(evtsp).eventSubProcess && classify(element).interrupting) {
      this._cancelScopeSiblings(source);
    }
  }

  // an **event-based gateway** resolves to the first of its catch events to fire: as this catch departs,
  // cancel the other branches' waiting tokens. Derived from the model (this catch's incoming flow is an
  // event-based gateway) and consumed internally, so it rides this `advanceToken` after the catch icon
  // and isn't a separate recorded action.
  if (ev === 'catch') {
    this._cancelEventGatewaySiblings(node, label);
  }

  // a **departing token sheds its children** — an activity's untriggered boundary listeners (and a
  // departing scope's event-sub waiters) become irrelevant the instant it departs, so they're shed
  // here, before the travel (a no-op for the common leaf token). The departing token is now childless.
...
// Fire a boundary event along its outflow. The boundary's dot becomes a **fresh** dot on the outflow,
// a child of the enclosing **scope** (the host's parent) — so it survives the host being cancelled,
// with no reparenting. **Interrupting**: cancel the host, which cascades to its whole subtree (the
// listener, the activity's inner tokens, and an MI activity's every sub-instance). **Non-interrupting**:
// leave the listener armed and flip it for feedback. Either way, travel the fresh dot to the far node.
// Reached only from `advanceToken({ sequenceFlow })` when the origin is a boundary event.
Animation.prototype._fireBoundary = async function(node, label, sequenceFlow, element) {
  const flow = this._elementRegistry.get(sequenceFlow);
  if (!flow || flow.source !== element) {
    throw new Error(`advanceToken: "${sequenceFlow}" is not an outflow of boundary "${node}"`);
  }
  const listener = this.getEntry(node, label);
  if (!listener) {
    throw new Error(`advanceToken: no token <${label}> at boundary <${node}>`);
  }

  // the boundary flies its symbol IN to the listener before it fires — awaited
  await this._primitives.catchIcon(node, label, { stackIndices: listener.stackIndices });

  const interrupting = classify(element).interrupting;
  const hostId = classify(element).attachedTo;
  const hostEntry = this.getEntry(hostId, label);
  const scopeToken = this._parentTokenOf(hostEntry ? hostEntry.token : listener.token);
  const stackIndices = listener.stackIndices;
  const color = listener.token.color;

  if (!interrupting) {
    // the listener stays armed; flip it for feedback (fire-and-forget)
    this._primitives.playTokenEffect(node, label, 'flip', { stackIndices });
  }

  // a fresh dot resting on the outflow, a child of the scope — created via primitives directly, so it
  // has no entrance gesture (the boundary dot appears to continue rather than pop in)
  const fresh = this._primitives.createToken(node, label, color, { sequenceFlow }, stackIndices);
  this._register(node, label, fresh, stackIndices, scopeToken, null, sequenceFlow);

  if (interrupting && hostEntry) {
    // cancel the host (cascades to the listener + the activity's whole subtree, MI subs included). For
    // an MI host the token is parked on its incoming flow, so select it by that flow. Fire-and-forget:
    // the model drops synchronously, the flip-fade plays while the fresh dot travels.
    this._consumeAt(hostId, label, hostEntry.sequenceFlow || undefined);
  }

  // travel the fresh dot to the far node (it already rests on the flow)
  const travelling = this._primitives.sendToken([ { node, label, sequenceFlow, stackIndices } ]);
  const [ landed ] = await travelling;
  this._replaceToken(fresh, landed);
  this._tokenMap.delete(fresh);
  this._register(landed.node, label, landed, landed.stackIndices, null, null, landed.state.sequenceFlow);
...
Animation.prototype.forkToken = async function(args) {
  ...
  // a fork inherits the original's selection, so forking a selected token (e.g. an interactive
  // inclusive fork) selects every outgoing branch, not just the first
  if (sibling.selected) {
    this._primitives.selectToken(node, label, { sequenceFlow, stackIndices: clone.stackIndices });
  }
...
Animation.prototype.joinTokens = async function(args) {
  ...
  const wasSelected = branches.some(b => b.token.selected); // a merge inherits the branches' selection
  ...
  if (wasSelected) {
    this._primitives.selectToken(node, label, { stackIndices });
  }
...
Animation.prototype.consumeToken = async function(args) {
  ...
  return this._consumeAt(node, args.label, args.sequenceFlow);
};

// The body of `consumeToken`, callable internally without being recorded as a separate log action —
// an interrupting boundary fire in `advanceToken` cancels the host this way. `node` is already canonical.
Animation.prototype._consumeAt = async function(node, label, sequenceFlow) {
  ...
  for (const [ stackedNode, key ] of Object.entries(target.stackIndices || {})) {
    ...
    // read/write the stack in the CONSUMED token's own context (its ancestor keys), not the current
    // front — so the key is dropped from the bucket it was actually added to (see the matching context
    // at creation). The context is the token's stackIndices minus this node's own key.
    const ctx = { ...target.stackIndices };
    delete ctx[stackedNode];
    const keys = this._primitives.getStacks(stackedNode, ctx);
    ...
    this._primitives.setStacks(stackedNode, this._primitives.getStacks(stackedNode, ctx).filter(k => k !== key), ctx);
...
Animation.prototype._unparkMIParent = function(node, element, parentToken) {
  ...
  if (!outFlow) {
    // No outgoing flow (the MI activity is the last node — implicit end — or an ad-hoc child). Un-park the
    // parent at the activity's **completion** position and reveal it — the analogue of releasing it onto an
    // outflow — so the host can finish it (consume). A hidden token doesn't glide, so the move is invisible;
    // it appears already at completion. `_departMIParent` then sees it off the inflow and consumes it.
    this._primitives.setState(pe.node, pe.label, { position: positionFor(Position.COMPLETION) },
      { sequenceFlow: pe.sequenceFlow || undefined, stackIndices: pe.stackIndices });
    pe.sequenceFlow = null;
    pe.position = Position.COMPLETION;
    this._primitives.setState(pe.node, pe.label, { hidden: false }, { stackIndices: pe.stackIndices });
    return;
  }
...
/**
 * Dim / undim a standard-loop activity's **loop marker** — pass-through to `primitives`. The simulator
 * uses this at a loop activity's completion: black = the token loops again, dimmed = it leaves the loop.
 */
Animation.prototype.setLoopMarkerDimmed = function(node, dimmed = true) {
  this._primitives.setLoopMarkerDimmed(node, dimmed);
};

/**
 * Enable / disable the clickable loop/exit **toggle** over a loop activity's marker — pass-through to
 * `primitives`. While on, clicking the marker fires `loop.marker.click` `{ node }`.
 */
Animation.prototype.setLoopToggleEnabled = function(node, on = true) {
  this._primitives.setLoopToggleEnabled(node, on);
};
...
/**
 * Reveal a token: bring its instance to the front on every stacked ancestor recorded in the
 * token's `stackIndices`, so a token hidden behind other instances becomes the visible front one.
 *
 * @param {Token} token
 * @return {Promise<void>}
 */
Animation.prototype.reveal = function(token) {
  // Revealing a token is making it visible, and that is two things, not one. Which instance it belongs to
  // is settled by bringing each stack in its chain to the front; which plane it lives on is settled by
  // drilling there. A token inside a collapsed sub-process rests on a plane of its own, so a caller acting
  // on a token it revealed — selecting it from a list, advancing it — would otherwise act on something the
  // reader is not looking at, and the act would appear to do nothing at all.
  //
  // The plane comes first: a stack is drawn on the plane its node is on, so bringing an instance to the
  // front is worth watching only once the reader is there to see it.
  this._primitives.drillTo(token && token.node);

  const stackIndices = (token && token.stackIndices) || {};

  return Promise.all(
    Object.keys(stackIndices).map(node => this._primitives.moveToFront(node, stackIndices[node]))
  );
};
...
// A departing token sheds its children — boundary listeners and event-sub waiters belong to it at rest
// and don't travel. They are dependents, garbage-collected the moment their owner leaves:
// each child subtree flip-fades out on its own ghost (the standard exit, skipped at duration 0), the same
// gesture a consume gives — so a host never records these removals; they follow from the owner's depart.
// A no-op for a leaf token.
Animation.prototype._shedChildren = function(token) {
  const effects = this._primitives.getAnimationDuration() > 0 ? EXIT : undefined;
  for (const child of this.getChildren(token).slice()) {
    this._tearDown(child, effects);
  }
};

// An interrupting event sub-process firing cancels every **other** token of its enclosing scope (the
// firing itself is excluded). Each is consumed through the internal `_consumeAt`, so the cancel is not a
// separate recorded action — it rides the firing's `advanceToken`, the same way a boundary's host cancel
// does. Fire-and-forget: the model drops synchronously and the flip-fades play out on their own.
Animation.prototype._cancelScopeSiblings = function(firing) {
  const scope = this._parentTokenOf(firing);
  if (!scope) {
    return;
  }
  for (const t of this.getChildren(scope).slice()) {
    if (t === firing) {
      continue;
    }
    const e = this._tokenMap.get(t);
    if (e) {
      this._consumeAt(e.node, e.label, e.sequenceFlow || undefined);
    }
  }
};

// An event-based gateway races its catch events: the first to fire wins and the rest are cancelled.
// Called as the winning catch (`node`) departs — find its source gateway, and if it is event-based,
// consume the `label` token waiting at every other branch's target. Internal (`_consumeAt`), so it is
// part of this `advanceToken`, not a separate recorded action.
Animation.prototype._cancelEventGatewaySiblings = function(node, label) {
  const element = this._requireElement(node);
  const inflow = (element.incoming || []).find(f => is(f, 'bpmn:SequenceFlow'));
  const gateway = inflow && inflow.source;
  if (!gateway || classify(gateway).gateway !== 'eventBased') {
    return;
  }
  for (const out of (gateway.outgoing || [])) {
    if (!is(out, 'bpmn:SequenceFlow') || !out.target || out.target.id === node) {
      continue;
    }
    const e = this.getEntry(out.target.id, label);
    if (e) {
      this._consumeAt(e.node, e.label, e.sequenceFlow || undefined);
    }
  }
