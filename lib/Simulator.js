...
Simulator.prototype.triggerCatchEvent = async function(node, label) {
  ...
  // the flow first). An **intermediate catch** just departs and continues.
  if (c.event === 'boundary') {
...
Simulator.prototype._departAndContinue = async function(node, label, afterDepart, onDeparted) {
  ...
  // `afterDepart` (cancel the interrupted scope) and `onDeparted` (re-arm a waiter) both run as the token
  // leaves the node, via advanceToken's `onDeparted` hook — fired right after the source node re-renders,
  // so the token is already off the start (a scope-sibling cancel excludes it by reference; a re-arm's
  // new waiter isn't wiped by the source re-render).
  const hook = (afterDepart || onDeparted)
    ? () => { if (afterDepart) { afterDepart(); } if (onDeparted) { onDeparted(); } }
    : undefined;
  const landed = await this._animation.advanceToken({ node, label, sequenceFlow: out.id, onDeparted: hook });
  return this.advanceToEntry({ node: landed.node, label: landed.label });
};
