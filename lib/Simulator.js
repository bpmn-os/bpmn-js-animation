...
Simulator.prototype.spawnInstance = async function(processId, startEventId) {
  ...
  // label new instances "<process>_k" with a per-process counter (k): the process name if set, else
  // the process id — always the process, never the participant (see _processOf)
  const processBo = this._processOf(processId);
  const name = processBo.name || processBo.id;
