...
Simulator.prototype._completedSubProcess = function(node, label) {
  ...
  if (!token || this._isArmedWaiter(token)) {
    return null; // consuming a **listener** (a shed boundary / event-sub waiter) never completes a scope
  }
  const parent = this._simulation.getParent(token);
  if (!parent) {
    return null;
  }
...
Simulator.prototype._consumesLastToken = function(root, node, label) {
  const token = this._simulation.getToken(node, label);
  if (!token || this._isArmedWaiter(token)) {
    return false; // consuming a listener (a shed boundary / event-sub waiter) never finishes the instance
  }
