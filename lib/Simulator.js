...
Simulator.prototype._jumpLink = function(node, label, link) {
  ...
  if (catches.length === 0) {
    return this._consume(node, label, true); // a dead-end throw — nothing catches this link
  }
  if (catches.length === 1) {
...
Simulator.prototype._killScopeSiblings = function(scope, firing) {
  ...
  for (const t of this._animation.getChildren(scope).filter(c => c !== firing)) {
    ...
    this._run(this._consume(t.node, t.label, true, { sequenceFlow: t.state.sequenceFlow }));
  }
};
...
Simulator.prototype._shedEventSubWaiters = function(scopeToken) {
  ...
  for (const t of this._animation.getChildren(scopeToken).filter(c => this._isEventSubWaiter(c))) {
    this._run(this._animation.consumeToken({ node: t.node, label: t.label, gesture: true }));
  }
};
...
Simulator.prototype._propagate = function(node, label, kind, code) {
  ...
  if (caught && caught.interrupting) {
    this._run(this._animation.consumeToken({ node, label, gesture: true }));
  } else {
...
Simulator.prototype.triggerCatchEvent = async function(node, label) {
  ...
  const afterDepart =
    ...
    c.event === 'boundary' ? () => this._animation.createToken({ node, label, animate: 'bounce' }) :
    null;
  return this._departAndContinue(node, label, afterDepart);
};
...
Simulator.prototype._consume = function(node, label, gesture, selector) {
  ...
  const tokenDone = this._animation.consumeToken({ node, label, gesture, sequenceFlow: selector && selector.sequenceFlow });
  ...
  return tokenDone
    ...
    .then(() => this._animation.consumeToken({ node: scope.node, label: scope.label, gesture }));
};
...
Simulator.prototype._terminate = function(label) {
  ...
  this._run(this._animation.consumeToken({ node: inst.processId, label, gesture: true }));
};
...
Simulator.prototype._consumeMISub = async function(node, subLabel, element) {
  ...
  await this._animation.consumeToken({ node, label: subLabel, gesture: true });
