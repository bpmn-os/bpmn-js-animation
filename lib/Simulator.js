...
Simulator.prototype.advanceToEntry = async function(args) {
  ...
  if (c.profile === 'event') {
    ...
    if (c.event === 'end') {
      ...
      await this._simulation.advanceToken({ node, label });          // center/none
      await this._eventIcon(node, label, element);                   // the icon flies out (no-op if none)
      return this.advanceToDeparted({ node, label });
    }
...
// Predict whether consuming `(node, label)` empties its enclosing **sub-process** scope — if so the
// sub-process body has finished. Returns `{ node, label }` of that sub-process token, else null.
// (Computed **before** the consume, while the token is still its parent's child.)
Simulator.prototype._completedSubProcess = function(node, label) {
  const token = this._simulation.getToken(node, label);
  const parent = token && this._simulation.getParent(token);
  if (!parent) {
    return null;
  }
  const el = this._elementRegistry.get(parent.node);
  const c = el && classify(el);
  if (!c || !c.subProcess || c.eventSubProcess) {
    return null;
  }
  // the body is finished when every other child is an **armed event-sub waiter** (a token resting at
  // a start event) — those don't count toward completion (the host fires them by choice)
  const active = this._simulation.getChildren(parent).filter(c => c !== token && !this._isArmedWaiter(c));
  if (!active.length) {
    return { node: parent.node, label: parent.label };
  }
  return null;
};
...
// where it awaits a double-click to depart its outflow (the same gesture as a task).
Simulator.prototype._onSubProcessComplete = function(sp) {
  const spToken = this._simulation.getToken(sp.node, sp.label);
  if (spToken) {
    this._removeArmedWaiters(spToken); // the body's leftover armed event-sub waiters don't survive
  }
...
// Drill back **out** to the plane the sub-process's shape lives on.
Simulator.prototype._drillOut = function(node, c) {
  if (!c.collapsed || !this._canvas.setRootElement || !this._canvas.findRoot) {
    return;
  }
...
Simulator.prototype._fireEventSub = function(node, label, element) {
  const evtsp = this._shapeOf(element.parent);
  const c = evtsp && classify(evtsp);
  if (!c || !c.eventSubProcess) {
    return this.advanceToDeparted({ node, label }); // an ordinary start event — just depart
  }
  ...
  if (c.interrupting) {
    this._killScopeSiblings(scope, firing); // interrupt: every other token of the scope dies
  } else if (scope) {
    this._simulation.createToken({ node, label: this._firingLabel(scope.label), animate: 'bounce' }); // re-arm
  }

  return this.advanceToDeparted({ node, label }); // run this firing through the event-sub body
};
...
// Tear down a scope's leftover **armed** event-sub waiters when it completes (a sub-process that
// reaches completion — a consumed process root cascades its whole subtree, so this isn't needed there).
Simulator.prototype._removeArmedWaiters = function(scopeToken) {
  for (const t of this._simulation.getChildren(scopeToken).filter(c => this._isArmedWaiter(c))) {
...
Simulator.prototype._propagate = function(node, label, kind, code) {
  const boundary = this._findCatchingBoundary(node, label, kind, code);
  if (boundary) {
    return this.triggerCatchEvent(boundary, label);
  }
  return this._consume(node, label, true); // uncaught — the throwing path ends
};

// Walk the scope hierarchy from the throwing event outward, returning the id of the first **armed**
// boundary event (on an enclosing activity) that matches `kind`/`code`, or null. Bubbles past a scope
// with no match to its enclosing scope (an error in a sub-process → the sub-process's boundary, else up).
Simulator.prototype._findCatchingBoundary = function(node, label, kind, code) {
...
Simulator.prototype.triggerCatchEvent = async function(node, label) {
  ...
  // event-based gateway race: triggering this catch event **wins** — flip-fade the sibling branches
  // (the instance's tokens still waiting at the gateway's other catch events) before it proceeds
  this._cancelEventBasedSiblings(node, label);
...
Simulator.prototype._consume = function(node, label, gesture) {
  // compute scope predicates **before** the consume (while the token is still its parent's child)
  const sp = this._completedSubProcess(node, label);
  ...
  // the consumed token was its sub-process's last child → that sub-process body has finished: glide it
  // to completion (drilling back out) once the token is gone
  if (sp) {
    return tokenDone.then(() => this._onSubProcessComplete(sp));
  }
  ...
  // The instance is finishing. Wait the last token's consume out **before** completing the box —
  // otherwise that leaf consume's stack-decrement (gated on the *leaf's* fade) sees the box token
  // already torn down by a concurrent box consume and drops the stack mid-fade (the box appears to
  // `moveToBack` after the flip, before the fade). Sequencing keeps the box drop owned by the box's
  // own consume, gated on the box's own flip-fade.
  this._instances.delete(label);
  return tokenDone
    .then(() => this._simulation.advanceToken({ node: inst.processId, label, position: Position.COMPLETION, animate: null }))
    .then(() => this._simulation.consumeToken({ node: inst.processId, label, gesture }));
};
...
Simulator.prototype._consumesLastToken = function(root, node, label) {
  ...
  // armed event-sub waiters (tokens at start events) don't count — they're torn down on completion
  return children.every(c => c === token || this._isArmedWaiter(c));
};
