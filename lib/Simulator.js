 ...
 *
 * **Not yet supported** (acceptable gaps, surfaced by the collaboration example): **sequential**
 * multi-instance (MI is parallel-only — each double-click spawns a concurrent sub; a sequential MI
 * should run one instance at a time up to its cardinality); **ad-hoc** sub-processes (their inner
 * activities run, but with no ad-hoc ordering/completion-condition semantics); and **message
 * delivery** — message flows are **never traversed** (a token only follows sequence flows), so a
 * throw/receive across pools doesn't actually pass a token between them.
 */
