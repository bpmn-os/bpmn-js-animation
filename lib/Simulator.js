...
Simulator.prototype.advanceToEntry = async function(args) {
  ...
  if (c.profile === 'event') {
    if (c.event === 'throw') {
      ...
      // token departs), then depart. A **link** throw skips the icon here: it crosses the link in
...
// fresh one appears at the catch. The consume uses the plain `animation.consumeToken` verb rather than
