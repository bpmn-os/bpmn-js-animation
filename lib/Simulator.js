...
export default function Simulator(config, eventBus, animation, primitives, elementRegistry, canvas) {
  ...
  eventBus.on('loop.marker.click', e => this._onLoopMarkerClick(e.node));
...
Simulator.prototype._reset = function() {
  this._counters = new Map();        // processId -> instance count (labels "<process>°k", per process)
 ...
 * @param {string} [instanceLabel]  a caller-supplied instance name (label); defaults to "<processId>°k"
 * @return {Promise<string>} the new instance label
 */
Simulator.prototype.spawnInstance = async function(processId, startEventId, instanceLabel) {
  ...
  // label: a caller-supplied instance name, or an auto "<processId>°k" per-process counter — the degree
  // sign (not "_") so it reads clearly against the modeller's underscore-style ids (e.g. Process_1°2),
  // always the process id (like we always use node ids), never the name and never the participant
  const processBo = this._processOf(processId);
  let label = instanceLabel;
  if (label) {
    if (this._instances.has(label) || this._animation.getToken(processId, label)) {
      throw new Error(`spawnInstance: an instance named <${label}> already exists`);
    }
  } else {
    const k = (this._counters.get(processBo.id) || 0) + 1;
    this._counters.set(processBo.id, k);
    label = processBo.id + '°' + k;
  }
  ...
  this._animation.createToken({ node: start.id, label, parentNode: processId, parentLabel: label });
  ...
  // the instance is running → the process box advances to busy, and rests there
  await this._animation.advanceToken({ node: processId, label, position: Position.BUSY, animate: null });
...
// The next free auto-style instance label "<processId>°k" — for a host (e.g. the Tokens panel's
// Instantiate field) to pre-fill a sensible, unique name. Does not reserve it (no side effects).
Simulator.prototype.suggestInstanceLabel = function(processId) {
  const base = this._processOf(processId).id;
  let k = 1;
  while (this._instances.has(base + '°' + k) || this._animation.getToken(processId, base + '°' + k)) {
    k++;
  }
  return base + '°' + k;
};
...
/**
 * Advance an activity/container token to `busy` — still for a process or sub-process, `busy/bounce` for a
 * task or call activity.
 *
 * A container that is busy is waiting for what is inside it, and waiting is not something to be shown a
 * cue for: a cue asks the reader to act, and there is nothing to do with a container but wait. What does
 * ask is `bounce`, which says a token may be advanced, and `pulse-pause`, which says a choice is the
 * reader's; a container rests, and the tokens moving within it are what says it is running.
 *
 * @param {{ node: string, label: string }} args
 */
Simulator.prototype.advanceToBusy = async function(args) {
  ...
  const element = this._requireElement(node);
  const c = classify(element);
  const animate = (c.process || c.subProcess) ? null : 'bounce';
...
Simulator.prototype.advanceToCompletion = async function(args) {
  ...
  // a standard-loop activity offers a loop/exit choice at completion. By default the token **loops**
  // (marker black, outflows dimmed); the user **clicks an outflow** to leave via it, or **clicks the
  // loop marker** to leave with no specific pick (any outflow, or — with no outflow — an implicit end).
  // The double-click then re-enters (loop) or departs (exit). Works with or without an outflow.
  if (c.loop) {
    const fb = { label, flows: new Set(), exitAny: false };
    this._chosen.set(node, fb);
    this._animation.setLoopToggleEnabled(node, true); // make the marker clickable
    this._paintLoopFallback(element, fb);
  }
};
...
Simulator.prototype._onElementClick = function(event) {
  ...
  if (classify(gateway).loop) {
    // a loop activity at completion: clicking an outflow means "leave via this flow" (one outflow, so
    // sole-select); clears any prior pick / exit-any. The loop marker dims to show the token will leave.
    fb.flows.clear();
    fb.flows.add(flow.id);
    fb.exitAny = false;
    this._paintLoopFallback(gateway, fb);
    return;
  }
...
// A click on a loop activity's loop marker toggles its decision between **looping** (default — marker
// black, outflows dimmed) and **leaving** (marker dimmed, outflows black / implicit end). Clicking it
// again returns to looping. Clears any specific outflow pick. The double-click then acts on this state.
Simulator.prototype._onLoopMarkerClick = function(node) {
  if (!this._active) {
    return; // inactive (model mode / playback)
  }
  const element = this._elementRegistry.get(node);
  const fb = element && this._chosen.get(node);
  if (!fb) {
    return; // no active loop decision at this node
  }
  if (fb.exitAny || fb.flows.size > 0) {
    fb.exitAny = false; // currently leaving → back to looping
    fb.flows.clear();
  } else {
    fb.exitAny = true; // currently looping → leave (any outflow / implicit end)
  }
  this._paintLoopFallback(element, fb);
};
...
// Paint a standard-loop activity's loop/exit decision: outflows dim unless the token is leaving via them
// (a specific picked flow, or **all** of them when "exit any"); the loop marker is black while looping
// and dimmed while leaving. Mirrors the gateway Fallback, plus the marker cue.
Simulator.prototype._paintLoopFallback = function(element, fb) {
  const leaving = fb.exitAny || fb.flows.size > 0;
  for (const flow of this._outgoing(element)) {
    this._animation.setFlowDimmed(flow.id, !(fb.exitAny || fb.flows.has(flow.id)));
  }
  this._animation.setLoopMarkerDimmed(element.id, leaving);
};

// Restore a loop activity: outflows to full opacity, loop marker un-dimmed, and the toggle hit removed.
Simulator.prototype._clearLoopFallback = function(element) {
  this._chosen.delete(element.id);
  for (const flow of this._outgoing(element)) {
    this._animation.setFlowDimmed(flow.id, false);
  }
  this._animation.setLoopToggleEnabled(element.id, false); // also un-dims the marker
};
...
Simulator.prototype._jumpTo = function(fromNode, label, catchEl) {
  ...
  // the catch is a child of the same enclosing scope as the throw (same instance)
  this._animation.createToken({ node: catchEl.id, label, parentNode: this._scopeOf(catchEl).id, parentLabel: label });
...
Simulator.prototype._spawnBoundaries = function(node, label) {
  ...
  for (const attacher of (element.attachers || [])) {
    ...
    this._animation.createToken({ node: attacher.id, label, animate: 'bounce', parentNode: node, parentLabel: label });
  }
};
...
Simulator.prototype._enterSubProcess = function(node, label, element, c) {
  ...
  if (c.adHoc) {
    ...
    for (const a of activities) {
      ...
      this._animation.createToken({ node: a.id, label, animate, parentNode: node, parentLabel: label });
    }
    return;
  }
  ...
  this._animation.createToken({ node: starts[0].id, label, parentNode: node, parentLabel: label });
...
Simulator.prototype._armEventSubs = function(scopeNode, label) {
  for (const evtsp of this._eventSubsOf(scopeNode)) {
    ...
    if (start) {
      // the firing is a child of the enclosing scope instance (the scope whose start event fired)
      this._animation.createToken({ node: start.id, label: this._firingLabel(label, evtsp), animate: 'bounce', parentNode: scopeNode, parentLabel: label });
    }
  }
};
...
Simulator.prototype._fireEventSub = async function(node, label, element) {
  ...
  const afterDepart = c.interrupting
    ? null
    : () => scope && this._animation.createToken({ node, label: this._firingLabel(scope.label, evtsp), animate: 'bounce', parentNode: scope.node, parentLabel: scope.label });
...
Simulator.prototype._loopStep = function(node, label, element) {
  const fb = this._chosen.get(node);
  this._clearLoopFallback(element);

  if (fb && fb.flows.size > 0) {
    // leave via the **picked** outflow. The activity's untriggered boundary listeners are dependents,
    // shed automatically by the travel's `_shedChildren` (flip-fade), so nothing to do here.
    return this._travel(node, label, [ ...fb.flows ][0]);
  }

  if (fb && fb.exitAny) {
    // leave the loop with no specific pick: depart normally — travels the single outflow, or **consumes**
    // when there is none (an implicit end). This is the no-outflow exit the loop marker enables.
    return this.advanceToDeparted({ node, label });
  }

  // default: re-enter (another iteration) — a standard loop is **one** activity execution, so its
  // boundary events monitor every iteration: keep the armed listeners and glide back to entry
...
Simulator.prototype._miStep = function(node, label, phase, element) {
  ...
  if (phase === 'busy') {
    // A sub-instance of a multi-instance **sub-process** finishes when its own body does, exactly as a
    // plain sub-process finishes: `_completedScope` reaches it through the token's parent, whatever its
    // label, and completes it as the last child is consumed. A click on it therefore waits rather than
    // cutting its children short. A multi-instance **task** has no body and does step here.
    if (classify(element).subProcess) {
      return this._noop(node, label);
    }
    return this.advanceToCompletion({ node, label });
  }
...
Simulator.prototype._spawnMISub = function(node, parentLabel) {
  ...
  this._animation.createToken({ node, label: subLabel, animate: 'bounce', parentNode: node, parentLabel });
