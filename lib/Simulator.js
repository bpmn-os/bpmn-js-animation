 ...
 */
export default function Simulator(eventBus, simulation, elementRegistry, canvas) {
  this._simulation = simulation;
  this._elementRegistry = elementRegistry;
  this._canvas = canvas;
  this._eventBus = eventBus;
  // The simulator does **no node selection** — it drives tokens + flow dimming only. Host/native node
  // selection still works and is **stack-aware automatically**: the `animation` service registers an
  // `outline` provider that grows the native selection frame for stacked nodes (see AnimationAPI).

  this._reset();

  // the simulator owns **record**: wrap the `simulation` verbs it drives so every BPMN event is
  // captured (downloadable as a log, replayable by the `animator`). Off until `startRecording`.
  this._recording = null;
  this._installRecorder(simulation);
...
Simulator.$inject = [ 'eventBus', 'simulation', 'elementRegistry', 'canvas' ];

// --- record: capture the BPMN events driven through `simulation` as a flat, replayable log --------

// Wrap each recorded `simulation` method so a call appends a flat event object while recording is on.
// Wraps the shared instance, so it captures the simulator's own driving (and any host calls on it).
Simulator.prototype._installRecorder = function(simulation) {
  for (const method of RECORDED_METHODS) {
    const original = simulation[method].bind(simulation);
    simulation[method] = (...args) => {
...
Simulator.prototype._jumpTo = function(fromNode, label, catchEl) {
  ...
  this._simulation.jumpToken(fromNode, label, catchEl.id);                      // re-home in place
...
Simulator.prototype._paintLinkFallback = function(node) {
  ...
  for (const id of fb.candidates) {
    this._simulation.setNodeDimmed(id, id !== fb.target);
  }
};
...
Simulator.prototype._clearLinkFallback = function(node) {
  ...
  if (fb && fb.candidates) {
    for (const id of fb.candidates) {
      this._simulation.setNodeDimmed(id, false);
    }
  }
...
Simulator.prototype._spawnBoundaries = function(node, label) {
  ...
  for (const attacher of (element.attachers || [])) {
    ...
    if (this._simulation.getToken(attacher.id, label)) {
      continue;
    }
...
Simulator.prototype._propagate = function(node, label, kind, code) {
  ...
  if (caught && caught.interrupting) {
    this._run(this._simulation.consumeToken({ node, label, gesture: true }));
  } else {
...
Simulator.prototype._findCatch = function(node, label, kind, code) {
  ...
  const throwing = this._simulation.getToken(node, label);
  let scope = throwing && this._simulation.getParent(throwing);
  ...
  while (scope) {
    ...
    if (scopeEl) {
      ...
      for (const evtsp of this._eventSubsOf(scope.node)) {
        ...
        const firing = start && this._simulation.getChildren(scope).find(t => t.node === start.id);
        if (firing) {
      ...
      const boundary = (scopeEl.attachers || []).find(b =>
        this._eventMatches(b, kind, code) && this._simulation.getToken(b.id, scope.label));
      if (boundary) {
        return { kind: 'boundary', node: boundary.id, interrupting: classify(boundary).interrupting };
      }
    }
    scope = this._simulation.getParent(scope);
  }
  return null;
};
...
Simulator.prototype._orJoinReady = function(element, label) {
  ...
  const scopeToken = this._simulation.getParent(arrived[0]);
  const siblings = scopeToken ? this._simulation.getChildren(scopeToken) : [];
...
Simulator.prototype._consume = function(node, label, gesture, selector) {
  ...
  const tokenDone = this._simulation.consumeToken({ node, label, gesture, sequenceFlow: selector && selector.sequenceFlow });
...
Simulator.prototype._step = async function(node, label, sequenceFlow) {
  ...
  if (c.profile === 'activity') {
    ...
    if (phase === 'entry') {
      ...
      if (c.task === 'send') {
        this._run(this._simulation.throwIcon(node, label));
      }
    ...
    if (phase === 'busy') {
      ...
      if (c.task === 'receive') {
        return this._simulation.catchIcon(node, label).then(() => this.advanceToCompletion({ node, label }));
      }
