 ...
 * drilling back out, to await a depart). Event sub-processes are upcoming.
 */
...
Simulator.prototype._completedSubProcess = function(node, label) {
  ...
  const children = this._simulation.getChildren(parent);
  if (children.length === 1 && children[0] === token) {
