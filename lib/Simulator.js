...
import { RECORDED_METHODS, describeEvent } from './eventLog';
 ...
 */
export default function Simulator(eventBus, animation, elementRegistry, canvas) {
  ...
  // the simulator owns **record**: wrap the `animation` verbs it drives so every BPMN event is
...
Simulator.$inject = [ 'eventBus', 'animation', 'elementRegistry', 'canvas' ];
 ...
 * appended to a flat event log. View navigation and focus settings are not recorded. Replay the log
...
Simulator.prototype.advanceToEntry = async function(args) {
  ...
  if (c.profile === 'event') {
    if (c.event === 'throw') {
      ...
      // token departs), then depart. A **link** throw skips the icon here: it teleports in
      // `advanceToDeparted` → `_jumpTo`, which flings the throw icon at the actual jump (so an
      // ambiguous link throw waiting on a pick doesn't flash its icon before it has jumped).
...
Simulator.prototype._eventIcon = function(node, label, element) {
  if (is(element, 'bpmn:ThrowEvent')) {
    return this._animation.throwIcon(node, label);
  }
  if (is(element, 'bpmn:CatchEvent')) {
    return this._animation.catchIcon(node, label);
  }
...
// A **link throw** teleports its token to the matching **link catch** (same link name) — there is no
// sequence flow between them; the token is re-homed in place (identity, colour, instance preserved).
// One match → jump straight there. Several matches (invalid BPMN, but handled) reuse the diverging-
// gateway Fallback over the candidate **catch nodes**: they dim, the user clicks one to choose it, and
// a double-click on the throw then jumps to the chosen catch.
...
// Teleport the token to the link catch (identity preserved), then continue from the catch as if it
// had just been triggered. The throw's icon flies **out** at the source (fire-and-forget, while the
// token jumps); `advanceToDeparted` routes the landed token through `triggerCatchEvent`, which flies
// the catch's icon **in** and departs along the catch's outflow — so the catch icon isn't doubled here.
Simulator.prototype._jumpTo = function(fromNode, label, catchEl) {
  this._run(this._eventIcon(fromNode, label, this._requireElement(fromNode))); // throw icon flies out
  this._animation.jumpToken(fromNode, label, catchEl.id);                      // re-home in place
...
Simulator.prototype._shedBoundaries = function(node, label) {
  const element = this._elementRegistry.get(node);
  const armed = (element.attachers || []).filter(b => this._animation.getToken(b.id, label));
  return Promise.all(armed.map(b => this._consume(b.id, label)));
};
...
Simulator.prototype._enterSubProcess = function(node, label, element, c) {
  ...
  if (c.adHoc) {
    ...
    for (const a of activities) {
      this._animation.createToken({ node: a.id, label, animate: 'bounce' });
    }
    return;
  }
...
Simulator.prototype._armEventSubs = function(scopeNode, label) {
  for (const evtsp of this._eventSubsOf(scopeNode)) {
    ...
    if (start) {
      this._animation.createToken({ node: start.id, label: this._firingLabel(label), animate: 'bounce' });
    }
  }
};
...
Simulator.prototype._fireEventSub = async function(node, label, element) {
  ...
  const afterDepart = c.interrupting
    ...
    : () => scope && this._animation.createToken({ node, label: this._firingLabel(scope.label), animate: 'bounce' });
...
Simulator.prototype._killScopeSiblings = function(scope, firing) {
  ...
  for (const t of this._animation.getChildren(scope).filter(c => c !== firing)) {
    // a sibling may rest on a flow (e.g. an MI activity's parked outer-thread token) — target it by
    // its flow so the interrupt tears it down too (a plain consume only finds an anchored token)
    this._run(this._consume(t.node, t.label, { sequenceFlow: t.state.sequenceFlow }));
  }
};
...
Simulator.prototype._shedEventSubWaiters = function(scopeToken) {
  ...
  for (const t of this._animation.getChildren(scopeToken).filter(c => this._isEventSubWaiter(c))) {
    this._run(this._animation.consumeToken({ node: t.node, label: t.label }));
  }
};
...
// The flow-node shapes directly inside an element (its businessObject's flowElements, resolved to
// shapes). A **Participant** (pool) holds its flow elements on its `processRef`, so resolve through it.
Simulator.prototype._innerOf = function(node) {
  const el = this._requireElement(node);
  const bo = el.businessObject;
  const container = bo.processRef || bo; // a Participant → its process; everything else → itself
...
Simulator.prototype._armedFiringAt = function(scope, startNode, instanceLabel) {
  const scopeToken = this._animation.getToken(scope.id, instanceLabel);
  return scopeToken && this._animation.getChildren(scopeToken).find(t => t.node === startNode) || null;
};
...
Simulator.prototype.triggerCatchEvent = async function(node, label) {
  ...
  // fresh listener **`onDeparted`** — the instant the triggered token has left the boundary *and* the node
  // re-rendered, so the boundary shows briefly empty and the new listener's fade-in + flip plays cleanly
  // (a re-arm before the travel's source re-render would be wiped). **intermediate catch** → nothing extra.
  const afterDepart =
    c.event === 'boundary' && c.interrupting ? () => this._run(this._consume(c.attachedTo, label)) : null;
  const onDeparted =
    c.event === 'boundary' && !c.interrupting ? () => this._animation.createToken({ node, label, animate: 'bounce' }) : null;
  return this._departAndContinue(node, label, afterDepart, onDeparted);
};
...
Simulator.prototype._departAndContinue = async function(node, label, afterDepart, onDeparted) {
  const out = this._outgoing(this._requireElement(node))[0];
  if (out) {
    this._animation.departToken(node, label, out.id); // off the node (a boundary detaches here)
  }
  if (afterDepart) {
    afterDepart();
  }
  ...
  // `onDeparted` runs inside the travel, right after the source node re-renders (see advanceToken)
  const landed = await this._animation.advanceToken({ node, label, sequenceFlow: out.id, onDeparted });
...
Simulator.prototype._cancelEventBasedSiblings = function(node, label) {
  const element = this._elementRegistry.get(node);
  const inFlow = this._incoming(element)[0];
  ...
  for (const out of this._outgoing(gateway)) {
    const sibling = out.target;
    if (sibling && sibling.id !== node && this._animation.getToken(sibling.id, label)) {
      this._run(this._consume(sibling.id, label));
    }
  }
};
...
Simulator.prototype._step = async function(node, label, sequenceFlow) {
  ...
  if (c.profile === 'activity') {
    ...
    if (phase === 'entry') {
      // a **send** task flings its message icon as it starts working, then advances immediately
      // (fire-and-forget — the icon flies off the token while it glides to busy)
      if (c.task === 'send') {
        this._run(this._animation.throwIcon(node, label));
      }
    ...
    if (phase === 'busy') {
      ...
      // a **receive** task waits for its message — the catch icon flies **into** the token, and only
      // once it lands does the task advance to completion (unlike the send task's fire-and-forget)
      if (c.task === 'receive') {
        return this._animation.catchIcon(node, label).then(() => this.advanceToCompletion({ node, label }));
      }
...
Simulator.prototype._loopStep = function(node, label, element) {
  ...
  if (fb && fb.flows.size > 0) {
    // the loop is done: shed this activity's untriggered boundary listeners (the flip-fade plays on
    // the **children** only, never the activity token) — fire-and-forget, so the token departs
    // immediately rather than waiting the gesture out — then travel the selected outflow
    this._run(this._shedBoundaries(node, label));
...
Simulator.prototype._spawnMISub = function(node, parentLabel) {
  ...
  this._animation.createToken({ node, label: parentLabel + '/' + (n + 1), animate: 'bounce' });
};
