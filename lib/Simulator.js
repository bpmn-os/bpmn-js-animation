...
Simulator.prototype._cancelEventBasedSiblings = function(node, label) {
  ...
  for (const out of (gateway.outgoing || [])) {
    ...
    if (sibling && sibling.id !== node && this._simulation.getToken(sibling.id, label)) {
      this._consume(sibling.id, label, true);
    }
  }
};
...
Simulator.prototype._consume = function(node, label, gesture) {
  ...
  // model drops now (synchronously); the flip-fade ghosts play independently — guard the async tail
  this._run(this._simulation.consumeToken({ node, label, gesture }));

  if (!last) {
    return;
  }

  // the instance is finishing — glide the box to completion, then flip-fade-consume it (background)
  this._instances.delete(label);
  this._run(
    this._simulation.advanceToken({ node: inst.processId, label, position: Position.COMPLETION, animate: null })
