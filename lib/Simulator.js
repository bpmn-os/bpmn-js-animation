...
Simulator.prototype._onTokenDblClick = function(event) {
  ...
  // advancing a token selects it (selection rides the token as it moves)
  this._selectOnly([ { node, label, sequenceFlow } ]);
...
Simulator.prototype.spawnInstance = async function(processId, startEventId) {
  ...
  // label new instances "<process>_k" with a per-process counter (k), using the process/pool name
  const bo = process.businessObject;
  const name = (bo && (bo.name || bo.id)) || processId;
  const k = (this._counters.get(processId) || 0) + 1;
  this._counters.set(processId, k);
  const label = name + '_' + k;
