 ...
 * consume (D3) with process completion; **parallel gateways** (diverging forks every outflow,
 * converging joins once all branches arrive); and **boundary events** — armed when the activity
 * becomes busy, shed on its normal departure, and on firing flip + depart their outflow, then
 * cancel the host (interrupting) or re-arm (non-interrupting). The standard-loop re-entry, MI
 * activities, sub-process entry, event sub-processes, the event-based-gateway race, and the
 * exclusive/inclusive/complex outflow Fallback are upcoming slices.
 */
...
Simulator.prototype.advanceToDeparted = async function(args) {
  ...
  // a boundary event fires here (the user triggered its armed token) — a special departure
  if (c.event === 'boundary') {
    return this._fireBoundary(node, label, element, c);
  }
  ...
  if (outgoing.length === 1) {
    return this._travel(node, label, outgoing[0].id, c);
  }
...
// Fire a boundary event (the user double-clicked its armed token): depart along its outflow (no
// flip — the trigger is the double-click). The token detaches from its host as it departs, so it
// survives an interrupting host consume. Then **interrupting** → cancel the host activity and all
// its other children, concurrently with the travel; **non-interrupting** → re-arm a fresh listener
// as the triggered token starts travelling. The boundary path continues from the far node.
Simulator.prototype._fireBoundary = async function(node, label, element, c) {
  const host = c.attachedTo;
  const out = (element.outgoing || [])[0];

  // depart onto the outflow **synchronously** (detaches the boundary from its host) — it "starts
  // travelling" now, so the re-arm / host consume below act while it's already off the boundary node
  if (out) {
    this._simulation.departToken(node, label, out.id);
  }

  if (c.interrupting) {
    // interrupting → cancel the host + its other children **as the boundary starts travelling**
    // (concurrently, not after it lands); the boundary has left the host's subtree, so it survives
    const travelP = out ? this._simulation.advanceToken({ node, label, sequenceFlow: out.id }) : null;
    await this._consume(host, label, true);
  ...
  // non-interrupting → re-arm a fresh listener **as** the triggered token starts travelling (the
  // old one is on its flow now, so the anchored identity is free), then travel it onward
