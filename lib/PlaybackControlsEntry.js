...
export default function createPlaybackControlsEntry(options = {}) {
  ...
  const update = () => {
    ...
    runButton.innerHTML = state === 'playing' ? PAUSE_ICON : PLAY_ICON;
...
// Feather's, as the controls beside this one are drawn, so that every control a run is driven by is the
// same weight at the same size and only the shape tells them apart.
const FEATHER = (paths) =>
...
export const PLAY_ICON = FEATHER('<polygon points="5 3 19 12 5 21 5 3"/>');

const PAUSE_ICON = FEATHER('<rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/>');
