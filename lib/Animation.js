...
Animation.prototype._drawNodeOutline = function(node, element, gfx) {
  ...
  svgAttr(outline, {
    x: -offset,
    y: -offset,
    width: (element.width || 0) + offset * 2 + extent,
 ...
 * next (`'forward'`) or previous (`'backward'`) instance, animating the actual
 * stack elements in place (no clones). The recycling shape **arcs over the stack**:
 * it lifts up clear of the body, travels across to the far slot, and drops in.
 * **Forward**: the front lifts, arcs to the back (dropping in *behind*), while the
 * copies each slide one slot toward the front. **Backward** is the mirror (the
 * lowest copy arcs to the front, dropping in *on top*, while the front + copies
 * slide one slot back).
 *
 * Because every shape is an identical clone, the canonical reset at the end is
 * visually seamless. Purely visual: the stack size is unchanged. No-op (resolved)
 * if the node has no stack (or the Web Animations API is unavailable).
 *
 * @param {string} node element id
...
Animation.prototype.scrollStack = function(node, direction = 'forward') {
  ...
  const gfx = this._elementRegistry.getGraphics(element);
  const realFront = gfx && gfx.querySelector(':scope > .djs-visual'); // the real node (slot 0)
  const copies = gfx ? Array.from(gfx.querySelectorAll('.bts-stack-shape')) : [];

  if (!realFront || !copies.length || !realFront.animate) {
    return Promise.resolve();
  }
  ...
  const n = copies.length;                 // copies occupy slots 1..n; DOM order is [slot n … slot 1]
  const lift = -(element.height + 10);      // clear the body, plus a gap
  const tf = slot => `translate(${slot * off}px, ${slot * off}px)`;

  // Animate clones only: add a clone of the front (slot 0, carrying its children),
  // then hide the real front + its real children for the gesture. The clone at slot i
  // is `copies[n - i]`; slot 0 is the new front clone.
  const frontClone = this._cloneNodeVisual(element, gfx);
  gfx.insertBefore(frontClone, realFront); // on top of the copies
  const atSlot = slot => slot === 0 ? frontClone : copies[n - slot];

  const realChildren = gfx.parentNode && gfx.parentNode.querySelector(':scope > .djs-children');
  ...
  const recycler = atSlot(back ? n : 0); // bottom clone (backward) or front clone (forward)
  const fromS = back ? n : 0;
  const toS = back ? 0 : n;

  // the recycler arcs: up (lift), across + down/up by the stack depth, then drop in
  animate(recycler, [
  ...
  for (let i = 0; i <= n; i++) {
    if (i === fromS) {
      continue; // the recycler
    }
    const to = back ? i + 1 : i - 1;
    animate(atSlot(i), [ { transform: tf(i) }, { transform: tf(to) } ], 'ease-out');
  }

  // swap the recycler's paint order at the apex — *after* it has lifted clear of the
  // stack, *before* it drops in: behind everything (forward) or on top (backward).
  const reorder = setTimeout(() => {
    if (back) {
      gfx.insertBefore(recycler, realFront); // on top of the copies
    } else {
      gfx.insertBefore(recycler, gfx.firstChild); // behind everything
    }
  }, duration * 0.45);
  ...
  return new Promise(resolve => {
    const finish = () => {
      ...
      // restore the real front + its children, then rebuild the canonical stack
      // (setStackSize clears all .bts-stack-shape, including the front clone)
 ...
 * `.djs-visual` plus — for a **container** (sub-process/event sub-process) — a clone
 * of its child shapes + flows. Children live in a *sibling* `.djs-children` group in
 * absolute diagram coords, so the clone is wrapped in `translate(-x, -y)` to bring
 * them relative to the element origin (then the copy's own slot offset applies).
 * Interaction rects, ids and any nested stack/selection decorations are stripped;
 * tokens are overlays (separate DOM) and are never cloned.
 */
Animation.prototype._cloneNodeVisual = function(element, gfx) {
  ...
  if (childrenGfx && childrenGfx.childNodes.length) {
    ...
    childrenClone.querySelectorAll('.djs-hit, .bts-stack-shape, .bts-node-outline')
