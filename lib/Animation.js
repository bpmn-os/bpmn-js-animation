...
Animation.prototype.setNodeSelected = function(node, selected = true) {
  ...
  if (gfx && !domQuery('.bts-node-outline', gfx)) {
    const offset = 5;
    ...
    svgAttr(outline, {
      ...
      y: -offset,
      rx: 4,
      width: (element.width || 0) + offset * 2,
      height: (element.height || 0) + offset * 2,
      class: 'bts-node-outline'
    });
...
Animation.prototype.setStackSize = function(node, size) {
  ...
  for (let i = 1; i <= copies; i++) {
    const shape = this._cloneNodeVisual(visual);
...
Animation.prototype.scrollStack = function(node, direction = 'forward') {
  ...
  const gfx = this._elementRegistry.getGraphics(element);
  const front = gfx && gfx.querySelector(':scope > .djs-visual'); // the real node (slot 0)
  const shapes = gfx ? Array.from(gfx.querySelectorAll('.bts-stack-shape')) : [];

  if (!front || !shapes.length || !front.animate) {
    return Promise.resolve();
  }
  ...
  const n = shapes.length;                 // copies occupy slots 1..n; DOM order is [slot n … slot 1]
  ...
  const recycler = back ? shapes[0] : front; // bottom shape (backward) or top/front (forward)
  ...
  // everything else slides one slot toward the vacated origin
  if (!back) {
    shapes.forEach((s, k) => { const i = n - k; animate(s, [ { transform: tf(i) }, { transform: tf(i - 1) } ], 'ease-out'); });
  } else {
    animate(front, [ { transform: tf(0) }, { transform: tf(1) } ], 'ease-out');
    shapes.forEach((s, k) => { const i = n - k; if (i < n) animate(s, [ { transform: tf(i) }, { transform: tf(i + 1) } ], 'ease-out'); });
  }
  ...
  // Doing it now (not up front) avoids the copies suddenly painting over the front.
  const reorder = setTimeout(() => {
    if (back) {
      gfx.insertBefore(recycler, front.nextSibling); // on top of the copies
    } else {
  ...
  return new Promise(resolve => {
    const finish = () => {
      ...
      front.style.opacity = '';
      this.setStackSize(node, size); // canonical rebuild — seamless (identical shapes)
      resolve();
    };
...
/** A `<g class="bts-stack-shape">` wrapping a clone of `.djs-visual` (a static stack copy). */
Animation.prototype._cloneNodeVisual = function(visual) {
