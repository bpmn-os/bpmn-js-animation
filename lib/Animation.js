 ...
 *
 * `nestedStacks` is `[{ node, stackSize }, …]` — the nested stacks of the instance that
 * ends up at the **front** after the scroll (the host passes the right one per
 * direction). Those child stack sizes are **committed** to the real diagram (so it
 * lands on the next instance). Committing happens between the two snapshots,
 * synchronously, before the real node is hidden — so no intermediate paint / flash.
 * Omit `nestedStacks` for a plain visual scroll. No-op if the node has no stack (or no
 * Web Animations API).
 *
 * @param {string} node element id
...
Animation.prototype.scrollStack = function(node, direction = 'forward', nestedStacks) {
  ...
  // happens before we hide the real node below (no flash)
  if (nestedStacks) {
