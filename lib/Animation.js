...
Animation.prototype.scrollStack = function(node, direction = 'forward', nestedStacks) {
  ...
  // step the at-node token order: forward recycles the top to the back, backward brings
  // the last to the front (so the next top is what B and the landing show)
  if (rotateToken) {
  ...
  // the at-node token badge is an HTML overlay (separate layer) — hide it for the gesture
  // (the snapshot dot stands in), restore on finish. Read after the rotation so we hide
  // the freshly-rendered overlay. Node only — descendants are 3e. The "+k" marker is
  // stack-level (instance count is unchanged by a scroll, and it sits outside the stack
  // footprint) so it stays visible throughout.
  const hiddenOverlays = [];
  (this._nodeOverlays.get(node) || []).forEach(id => {
...
Animation.prototype._cloneNodeVisual = function(element, gfx, withContent) {
  ...
  // the node's own top token rides the snapshot (3c — at-node token only; scope/
  // descendant tokens are 3e)
...
/**
 * Draw the node's **own top token** (`_visibleTokensAt(node)[0]`) as an SVG dot into a
 * snapshot clone, so it rides the scroll arc. Element-local coordinates — same space as
 * the cloned `.djs-visual`. No-op if the node has no visible token. Descendant/scope
 * tokens are **not** drawn here (3e).
 */
Animation.prototype._drawTokenDots = function(group, element) {
  const top = this._visibleTokensAt(element.id)[0];

  if (!top) {
    return;
  }

  const p = this._clusterPoint(element, top.state);
