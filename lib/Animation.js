...
Animation.prototype.sendToken = function(transitions) {
  ...
  try {
    for (const transition of transitions) {
      ...
      let matches = this._find(node, label);

      // disambiguate by instance when several same-label tokens exist at the node
      ...
      const srcKey = this._key(node, label, token.state.sequenceFlow, token.stackIndices);
  ...
  for (const group of groups.values()) {
    ...
    const indices = group.token.stackIndices; // a move stays in the same instance
    this._tokens.delete(this._key(group.node, group.label, group.token.state.sequenceFlow, indices));
    ...
    for (const { connection, toNode, waypoints, state } of group.flows) {
      ...
      const destKey = this._key(toNode, group.label, state.sequenceFlow, indices);
      const existing = this._tokens.get(destKey);
      const selected = !!group.token.selected || !!(existing && existing.selected);

      const branch = { node: toNode, label: group.label, color: group.color, state, selected, stackIndices: indices };
 ...
 * completes); the instance (`stackIndices`) is unchanged.
 *
 * @param {string} node
 * @param {string} label
 ...
 * `size - 1` opaque copies peeking out behind it (shifted by `STACK_OFFSET`, capped at
 * `maxVisible`). Purely visual & host-driven; the library never infers the size from tokens.
 *
 * The size is declared **per ancestor-instance context**: `ancestorStackIndices` (a map
 * `{ stackedAncestorId: index }`) says *which outer instance this size applies to*, so a
 * nested activity can have a different count under each outer instance. **Contexts are
 * independent** — a size set for one outer instance does *not* leak to others; an unset
 ...
 * resolves against whichever instance is on screen. `size <= 1` removes it in that context.
 *
 * @param {string} node element id
 * @param {number} size number of instances (`<= 1` removes it for this context)
...
Animation.prototype._redrawStack = function(node) {
  ...
  // pool-style box and stack against that (T4)
...
Animation.prototype._cloneNodeVisual = function(element, gfx, withContent) {
  ...
  if (sources.length) {
    ...
    sources.forEach(src => {
      ...
      clone.querySelectorAll('.djs-hit, .bts-node-outline').forEach(el => el.remove());
 ...
 * tokens are drawn (matching what `_renderNode` shows / what lands).
 */
Animation.prototype._drawTokenDots = function(group, element) {
  ...
  for (const id of nodes) {
    ...
    this._visibleTokensAt(id).forEach(t => this._appendTokenDot(group, element, de, t));
  }
};
