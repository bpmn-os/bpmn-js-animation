...
Animation.prototype.scrollStack = function(node, direction = 'forward') {
  ...
  const collectOverlay = id => {
    ...
    if (o && o.html) {
      // a flow-cluster overlay (tokens resting on a sequence flow) is not tied to the
      // instance, so it stays put through the gesture rather than riding/hiding with it
      if (o.html.querySelector('.bts-token-count[data-sequence-flow]:not([data-sequence-flow=""])')) {
        return;
      }
 ...
 * tokens are drawn (matching what `_renderNode` shows / what lands). **Tokens resting on a
 * sequence flow are skipped** — a flow token isn't tied to the node's instance (its own
 * node's stack index doesn't gate it), so it must not ride the per-instance scroll snapshot.
 */
Animation.prototype._drawTokenDots = function(group, element) {
  const nodes = new Set();

  for (const token of this._tokens.values()) {
    if (!this._isVisible(token) || token.state.sequenceFlow) {
      continue;
    }
  ...
  for (const id of nodes) {
    ...
    this._visibleTokensAt(id)
      .filter(t => !t.state.sequenceFlow) // flow-resting tokens don't ride the snapshot
...
Animation.prototype._renderNode = function(node) {
  ...
  for (const tokens of clusters.values()) {
    ...
    // a token resting on a flow uses the smaller, moving-token size (so a dot in transit
    // and one parked on the flow look the same); anchor tokens use the full badge size
    const size = tokens[0].state.sequenceFlow ? MOVING_TOKEN_SIZE : DOT_SIZE;
...
Animation.prototype._dotHTML = function(token) {
  ...
  return `
    <div class="bts-token-count waiting${bounce ? ' bts-bounce' : ''}${token.selected ? ' bts-selected' : ''}${sequenceFlow ? ' bts-on-flow' : ''}"
