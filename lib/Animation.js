...
export default function Animation(config, eventBus, canvas, overlays, elementRegistry, outline) {
  ...
  this._stackSizes = new Map();        // node -> Map<contextKey, size>  (cache of the key count)
 ...
 *
 * **Count-based shim** over the key-based {@link Animation#updateStacks}: instances here are
 * keyed by their **numeric index `0..size-1`** (the host doesn't name them), preserving the
 * existing order's arrangement where it still fits. A caller that needs *named* instances uses
 * `updateStacks` with its own keys (e.g. SimulationAPI passes instance labels).
 *
 * @param {string} node element id
...
Animation.prototype.setStackSize = function(node, size, ancestorStackIndices) {
  ...
  const current = (orders && orders.get(this._contextKey(context))) || [];

  this.updateStacks(node, this._buildOrder(size, current), context);
};

/** Numeric instance keys `[0…size-1]`, preserving an existing order's arrangement where it fits. */
 ...
 * @param {Object<string,*>} [ancestorStackIndices] the outer-instance context (omit = current)
 */
Animation.prototype.updateStacks = function(node, keys, ancestorStackIndices) {
  this._requireElement(node);
...
Animation.prototype.getStacks = function(node) {
  this._requireElement(node);
 ...
 * (For count-based stacks the key *is* the index; for named stacks use `moveToFront(key)`.)
 *
 * @param {string} node
 * @param {number} index
 */
