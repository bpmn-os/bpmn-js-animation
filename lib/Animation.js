...
Animation.prototype.scrollStack = function(node, direction = 'forward', nestedStacks) {
  ...
  // the at-node token badge + "+k" marker are HTML overlays (separate layer) — hide them
  // for the gesture (the snapshot dots stand in), restore on finish. Read after the
  // rotation so we hide the freshly-rendered overlays. Node only — descendants are 3e.
  ...
  (this._nodeOverlays.get(node) || []).forEach(collectOverlay);
  collectOverlay(this._stackOverlays.get(node));
