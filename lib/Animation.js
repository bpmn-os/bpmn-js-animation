...
// `config.tokenAnimation.duration` or `tokens.setDuration()`. A long flow and a
// short flow take the same time.
const DEFAULT_DURATION = 1000;


/**
 * @param { { duration?: number } | null } [config]
...
export default function Animation(config, canvas, eventBus) {
  ...
  this._duration = config && config.duration != null ? config.duration : DEFAULT_DURATION;
...
/**
 * Set the fixed transition duration (ms) for subsequent animations.
 *
