 ...
 * the node's **own top token** (`_visibleTokensAt(node)[0]`, 3c) plus every **visible
 * scope token** resting on a descendant (3e). All in the clone's element-local space.
 */
Animation.prototype._drawTokenDots = function(group, element) {
  const top = this._visibleTokensAt(element.id)[0];

  if (top) {
    this._appendTokenDot(group, element, element, top);
  }

  for (const token of this._order) {
    if (token.node === element.id || !this._isVisible(token) || !this._isDescendant(token.node, element.id)) {
      continue;
    }
    ...
    if (de) {
      this._appendTokenDot(group, element, de, token);
    }
  }
};
