...
import { createCollapsibleEntry, createSimpleEntry, createSeparator } from 'bpmn-js-side-panel';
...
// `config.tokenPanel.renderTokenDetail` is the host's row body: given one, every row gains a caret and
// the host draws inside it (see createTokenEntry). The panel neither reads nor redraws that body itself.
TokenPanel.prototype._tokenListOptions = function() {
  return {
    renderDetail: this._config.renderTokenDetail,
    onClick: (token, event) => this._revealAndSelect(token, event),
    onDblClick: (token, event) => this._advanceToken(token, event),
...
/**
 * Say in the tab's name how many tokens are standing, so that a run can be followed while another tab is
 * open. The count is dropped when there are none, a tab reading "(0)" being noise rather than news.
 */
TokenPanel.prototype._updateTabName = function() {
  if (!this._sidePanel) {
    return;
  }

  const tokens = this._primitives.getTokens().length;

  this._sidePanel.setTabLabel('tokens', tokens ? this._tabName + ' (' + tokens + ')' : this._tabName);
};
...
TokenPanel.prototype._init = function() {
  ...
  // how many tokens are standing, said in the tab's own name, so that a run can be followed while another
  // tab is open. It is the name rather than a mark beside it because the name is what both views show: a
  // selector in one, a column's resizer in the other.
  this._sidePanel = sidePanel;
  this._tabName = this._config.label || 'Tokens';
  ...
  // the "(Shift-)double-click stacks" hint lives at the BOTTOM of the body — a simple entry above the
  ...
  const stackHint = createSimpleEntry();
  ...
  this._eventBus.on('token.added', e => { this._onTokenAdded(e.token); this._refreshSuggestion(); this._fitHints(); this._updateTabName(); });
  this._eventBus.on('token.removed', e => { this._onTokenRemoved(e.token); this._autoExpandInstantiate(); this._refreshSuggestion(); this._updateTabName(); });
  ...
  this._eventBus.on('tokens.cleared', () => { this._clearLists(); this._autoExpandInstantiate(); this._refreshSuggestion(); this._resetHints(); this._updateTabName(); });
...
TokenPanel.prototype._renderFooter = function(pane) {
  ...
  refresh.type = 'button';
  refresh.innerHTML = UNDO_SVG + ' Refresh';
  ...
  this._loadEl.innerHTML = FILE_UPLOAD_SVG + ' Load log';
  ...
  this._saveEl.innerHTML = FILE_DOWNLOAD_SVG + ' Save log';
...
// A token changed node → its row is keyed by the node it left, so it is re-keyed onto the node it
// reached rather than looked up under a key it no longer has. The row itself survives (with its
// expanded body), showing the new node tag, and the node list's membership is adjusted (the token may
// have left or entered an inspected node). `previous` is the moved token as the row was keyed: the
// token differs from its former self in its node alone.
TokenPanel.prototype._onTokenMoved = function(e) {
  const previous = { ...e.token, node: e.from };

  this._selectedTokens.rekey(previous, e.token);

  const toInspected = this._listsNode(e.to);
  if (this._nodeTokens.has(previous)) {
    if (toInspected) {
      this._nodeTokens.rekey(previous, e.token);  // moved within inspected nodes
    } else {
      this._nodeTokens.remove(previous);          // left the inspected node
    }
  } else if (toInspected) {
...
//
// The token is revealed first, exactly as a click from the panel reveals before selecting. A list shows
// every token of a run, including those resting on a plane the reader is not on and those belonging to an
// instance that is not at the front of its stack; advancing one without revealing it runs the step out of
// sight, which reads as a gesture that did nothing.
TokenPanel.prototype._advanceToken = function(token, originalEvent) {
...
TokenPanel.prototype._revealAndSelect = function(token, originalEvent) {
  // bring the token's stack(s) to the front, then select it (reuses the click-selection path). The
  // reader's own event goes with it, so that shift adds to the selection here as it does on the canvas.
  Promise.resolve(this._animation.reveal(token)).then(() => {
...
// The footer's icons. Each Font Awesome Free icon is taken as they publish it and carries their notice
// with it, which is how their licence is satisfied for an SVG embedded in source; none is modified. What
// is drawn here instead is drawn because the icon wanted was not to hand, and it is a stroke where theirs
// are filled paths, though the two read at one weight. Either way the colour is the button's own, so a
// hovered or disabled control carries its icon with it.
const FA = (inner) =>
  '<svg width="15" height="15" viewBox="0 0 640 640" fill="currentColor" aria-hidden="true" '
  + 'focusable="false" style="vertical-align:-3px">'
  + '<!--!Font Awesome Free v7.3.1 by @fontawesome - https://fontawesome.com License - '
  + 'https://fontawesome.com/license/free Copyright 2026 Fonticons, Inc.-->'
  + inner + '</svg>';

const FA_PATH = (path) => FA('<path d="' + path + '"/>');

const UNDO_SVG = FA_PATH(
  'M80 64C88.8 64 96 71.2 96 80L96 183.8L133 144.7C181.6 93.2 249.2 64 320 64C461.4 64 576 178.6 576 '
  + '320C576 461.4 461.4 576 320 576C221.7 576 136.4 520.6 93.5 439.5C89.4 431.7 92.4 422 100.2 417.9C108 '
  + '413.8 117.7 416.8 121.8 424.6C159.4 495.6 234.1 544 320 544C443.7 544 544 443.7 544 320C544 196.3 '
  + '443.7 96 320 96C258 96 198.8 121.5 156.2 166.6L117.1 208L224 208C232.8 208 240 215.2 240 224C240 '
  + '232.8 232.8 240 224 240L80 240C71.2 240 64 232.8 64 224L64 80C64 71.2 71.2 64 80 64z');

// Font Awesome's file icon is a sheet and an arrow, drawn as separate sub-paths of one path. The two the
// footer needs differ in the arrow alone, so the sheet is written once and the arrow is drawn as it comes
// for the download and mirrored about its own middle for the upload. That mirror is the one change made to
// anything of theirs, and it is recorded in LICENSE; drawing the second icon instead would have put a
// stroke of ours beside a filled path of theirs, which is two styles in one row of buttons.
const FILE_SHEET =
  'M192 96L320 96L320 192C320 227.3 348.7 256 384 256L480 256L480 512C480 529.7 465.7 544 448 544L192 '
  + '544C174.3 544 160 529.7 160 512L160 128C160 110.3 174.3 96 192 96zM352 109.3L466.7 224L384 224C366.3 '
  + '224 352 209.7 352 192L352 109.3zM192 64C156.7 64 128 92.7 128 128L128 512C128 547.3 156.7 576 192 '
  + '576L448 576C483.3 576 512 547.3 512 512L512 250.5C512 233.5 505.3 217.2 493.3 205.2L370.7 82.7C358.7 '
  + '70.7 342.5 64 325.5 64L192 64z';

const FILE_ARROW =
  'M236.7 419.3L308.7 491.3C314.9 497.5 325.1 497.5 331.3 491.3L403.3 419.3C409.5 413.1 409.5 402.9 403.3 '
  + '396.7C397.1 390.5 386.9 390.5 380.7 396.7L336 441.4L336 320C336 311.2 328.8 304 320 304C311.2 304 304 '
  + '311.2 304 320L304 441.4L259.3 396.7C253.1 390.5 242.9 390.5 236.7 396.7C230.5 402.9 230.5 413.1 236.7 '
  + '419.3z';

const FILE_DOWNLOAD_SVG = FA_PATH(FILE_SHEET + FILE_ARROW);

// the arrow spans 304 to 496, so mirroring about 400 turns it over where it stands
const FILE_UPLOAD_SVG = FA('<path d="' + FILE_SHEET + '"/>'
  + '<path transform="matrix(1 0 0 -1 0 800)" d="' + FILE_ARROW + '"/>');
