...
TokenPanel.prototype._renderFooter = function(pane) {
  ...
  refresh.type = 'button';
  refresh.innerHTML = REFRESH_ICON + ' Refresh';
  ...
  this._loadEl.innerHTML = LOAD_ICON + ' Load log';
  ...
  this._saveEl.innerHTML = SAVE_ICON + ' Save log';
...
// The footer's three icons are Feather's, which is what the applications hosting this
// panel draw with, so that saving a log and saving a diagram read as one act in two places. Feather is
// drawn for a two-pixel stroke on a twenty-four-pixel grid; at fifteen that renders a little under one and
// a quarter, so the stroke is thickened to keep the weight rather than the number.
const FEATHER = (paths) =>
