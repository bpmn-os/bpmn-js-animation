...
TokenPanel.prototype._renderInstantiateGroup = function() {
  const group = el('div', 'bjs-token-group');

  // header: title + a caret on the right that points right when collapsed and rotates to down when open
  // (the bpmn properties-panel group caret — same SVG path)
  const header = el('div', 'bjs-token-group-header');
  header.appendChild(text('span', 'bjs-token-group-title', 'Instantiate process'));
  const arrow = el('span', 'bjs-token-group-arrow');
  arrow.innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" aria-hidden="true">'
    + '<path fill-rule="evenodd" d="M10,12 L3,12 C2.44771525,12 2,11.5522847 2,11 '
    + 'C2,10.4477153 2.44771525,10 3,10 L8,10 L8,5 C8,4.44771525 8.44771525,4 9,4 '
    + 'C9.55228475,4 10,4.44771525 10,5 L10,12 Z" transform="rotate(-45 6 8)"></path></svg>';
  header.appendChild(arrow);
  group.appendChild(header);

  const entries = el('div', 'bjs-token-group-entries');
  ...
  // collapsed by default; the header toggles, repopulating the process list on open (the model may have
  // changed since the panel was built)
  ...
  const setOpen = open => {
    ...
    if (open) {
      this._populateProcessSelect(select); // the model may have changed since the panel was built
      if (!input.value) {
        suggest();                         // pre-fill a sensible name on open
      }
      btn.disabled = !input.value.trim() || !select.value;
    }
  };
  this._openInstantiate = setOpen; // let the panel auto-expand the group (e.g. when there are no tokens)
  header.addEventListener('click', () => setOpen(!group.classList.contains('open')));
