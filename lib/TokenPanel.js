...
import createControlsEntry from './ControlsEntry';
import { PLAY_ICON } from './PlaybackControlsEntry';
...
/**
 * Mount a consumer-provided control element into the panel's controls region, and return a handle to
 * remove it. Lets a consumer extend the panel with its own
...
TokenPanel.prototype._applyMode = function() {
  ...
  // The filter stays in the band in every mode. It belongs to the tab rather than to a run, so a band that
  // lost it would name the tab in one mode and name it half a line lower in another, and the reader would
  // watch the head of the tab change shape as the mode turned over.
  if (this._inspector) {
  ...
  // A driven simulation has no transport, no speed and nothing to read a log into: the entry is told which
  // it is and leaves out what would mean nothing, keeping what remains against the right edge.
  if (this._controls) {
    this._controls.setMode(play ? 'play' : 'simulate');
  }

  this._updateLogButton();           // whether the load control may act — see _updateLogButton
...
TokenPanel.prototype._updateLogButton = function() {
  ...
  if (this._controls) {
    this._controls.setLogButton(showSave ? 'save' : 'load');
  }
};
...
TokenPanel.prototype._init = function() {
  ...
  const { header, body } = sidePanel.addTab({
    id: 'tokens', label: this._config.label || 'Tokens', priority: 0
  });
  ...
  // The (all / selected) view filter, which changes WHAT is listed and never the selection itself. It
  // stands in the tab's band, under the tab's name, since it governs the whole list and so must stay while
  // the list scrolls.
  ...
  this._band.appendChild(text('h1', 'bjs-tab-name', this._tabName));
  this._band.appendChild(this._filterHeading);
  [
...
TokenPanel.prototype._renderControls = function(pane) {
  ...
  // Auto-focus is not here. It governs the canvas rather than this list, so a host draws it where its own
  // furniture over the canvas goes; the setting itself is the animator's, `animator.autoFocus` to write and
  // `animator.getAutoFocus` to read, announced as `autoFocus.changed`. A bar here would be a second writer
  // of one state, standing away from what it governs.
...
TokenPanel.prototype._renderInstantiateGroup = function() {
  ...
  // the play glyph the transport wears: what this does is start an instance, and a run is started with one
  btn.innerHTML = PLAY_ICON;
  btn.disabled = true;

  // suggest the next free auto-name "<process>°k" for the selected process — pre-filled so a single press
  // spawns immediately, and repeated presses fire successive instances. `isAuto` tracks whether the
...
// The controls a run is driven by, at the foot of the tab. They are one entry, `createControlsEntry`, and a
// host that keeps what governs the whole panel somewhere of its own — the panel's footer — says
// `controls: false` and mounts that entry there itself; the panel then draws none and the run still has one
// set of controls. What each part is shown in which mode is `_applyMode`.
TokenPanel.prototype._renderFooter = function(pane) {
  ...
  this._controls = createControlsEntry({
    ...
    onLoad: (log) => { this._loadedLog = log; }
  });

  this._playbackEl = this._controls.transport.element;

  footer.appendChild(this._controls.element);
