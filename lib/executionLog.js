 ...
 * `{ action, ...fields }`, where `action` is an `animation` method name and the fields are its named
 ...
 * the events themselves (run it with `autoFocus` on), so the log is a clean execution log, not a
...
export const POSITIONAL_FIELDS = {
  ...
  throwIcon: [ 'node', 'label' ],
  catchIcon: [ 'node', 'label' ]
};
