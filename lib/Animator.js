 ...
 */
export default function Animator(animation, primitives, eventBus, config) {
  this._animation = animation;
  this._primitives = primitives;
  this._eventBus = eventBus;

  // Follow the active instance and plane while replaying. It is a setting of the animator's own rather than
  // of whatever control a host draws for it: a host may draw one, or several, or none, and each of them
  // reads this and hears when it changes, so no two controls can drift apart.
  this._autoFocus = !config || config.autoFocus !== false;
  this._animation.autoFocus(this._autoFocus);
}

Animator.$inject = [ 'animation', 'primitives', 'eventBus', 'config.animator' ];

/**
 * Turn auto-focus on or off: when on, each event reveals its instance, and drills to its plane, before
 * animating, so the motion plays where it belongs. Forwards to `animation.autoFocus` so the functions' own
 * post-operation reveal fires too, and announces `autoFocus.changed` with the new value.
 *
 * It is on unless a host states `animator: { autoFocus: false }`, and a change to what it already is
 * announces nothing, so a control that writes what it reads sets nothing off.
 */
Animator.prototype.autoFocus = function(on = true) {
  const next = !!on;

  if (next === this._autoFocus) {
    return;
  }

  this._autoFocus = next;
  this._animation.autoFocus(next);
  this._eventBus.fire('autoFocus.changed', { autoFocus: next });
};

/** Whether auto-focus is on. */
Animator.prototype.getAutoFocus = function() {
