...
/**
 * Animator — an opinionated **playback** tool over the `simulation` service, the mirror of the
...
Animator.prototype.replay = async function(log, { gate } = {}) {
  ...
  for (const event of log) {
    ...
    if (startsImmediately(event)) {
      outstanding.push(Promise.resolve(eventCall(simulation, event)));
      continue;
    }
    ...
    if (this._autoFocus) {
      ...
      const token = node != null && simulation.getToken(node, event.label);
      if (token) {
        simulation.focusToken(token);
        await simulation.whenFocused();
      }
    }

    await eventCall(simulation, event);
