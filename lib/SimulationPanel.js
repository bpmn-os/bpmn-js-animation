...
SimulationPanel.prototype._init = function() {
  ...
  // an explicit canvas (root) click toggles the process context (click again to unselect); clicking
  // or deselecting a node clears it. Lower priority so the selection is already current.
...
// A token was created → add a row to the node list if it sits at a currently-inspected node.
SimulationPanel.prototype._onTokenAdded = function(token) {
  if (this._nodeIds.indexOf(token.node) !== -1 && !findByToken(this._nodeList, token)) {
...
SimulationPanel.prototype._onTokenMoved = function(e) {
  ...
  const toInspected = this._nodeIds.indexOf(e.to) !== -1;
...
// Node selection (or the process-context toggle) changed → update the node list incrementally: drop
// rows for nodes no longer inspected, add rows for tokens at newly-inspected nodes.
SimulationPanel.prototype._onNodeSelectionChanged = function() {
  ...
  if (!nodes.length && this._processContext && rootEl && is(rootEl, 'bpmn:Process')) {
    nodes = [ rootEl.id ]; // click-canvas → inspect the implicit process (like the properties panel)
  }
  this._frameProcessBox(rootEl, nodes);

  const prev = this._nodeIds;
  this._nodeIds = nodes;

  for (const row of Array.from(this._nodeList.children)) {
    if (nodes.indexOf(row.dataset.node) === -1) {
      this._nodeList.removeChild(row); // node no longer inspected
    }
  }
  nodes.forEach(node => {
    if (prev.indexOf(node) === -1) {
      this._primitives.getTokens(t => t.node === node).forEach(t => {
