...
SimulationPanel.prototype._renderInspector = function() {
  ...
  if (selectedTokens.length) {
    root.appendChild(text('div', 'bjs-sim-list-title', `Selected token${selectedTokens.length > 1 ? 's' : ''}`));
  ...
  // one combined list of all tokens at the selected node(s)
  if (nodes.length) {
    if (selectedTokens.length) {
      root.appendChild(el('div', 'bjs-sim-separator'));
    }
    root.appendChild(text('div', 'bjs-sim-list-title', 'Tokens at selected node(s)'));
    const nodeTokens = nodes.flatMap(node => this._primitives.getTokens(t => t.node === node));
    if (!nodeTokens.length) {
      root.appendChild(hint('No tokens at the selected node(s).'));
    } else {
      const list = el('ul', 'bjs-sim-list');
      nodeTokens.forEach(token => list.appendChild(this._tokenRow(token)));
      root.appendChild(list);
    }
  }
