...
SimulationPanel.prototype._init = function() {
  ...
  this._selectedTitle = text('div', 'bjs-sim-list-title', 'Selected tokens');
  this._selectedList = el('ul', 'bjs-sim-list');
  this._separator = el('div', 'bjs-sim-separator');
  this._nodeTitle = text('div', 'bjs-sim-list-title', 'Tokens at selected node(s)');
  this._nodeList = el('ul', 'bjs-sim-list');
  ...
  this._eventBus.on('token.added', e => this._onTokenAdded(e.token));
  this._eventBus.on('token.removed', e => this._onTokenRemoved(e.token));
...
SimulationPanel.prototype._renderFooter = function(pane) {
  ...
  const row = el('div', 'bjs-sim-footer-row');
  ...
  this._loadEl = el('button', 'bjs-sim-btn');
  ...
  if (this._simulator) {
    this._saveEl = el('button', 'bjs-sim-btn');
...
SimulationPanel.prototype._updateRunButton = function() {
  ...
  this._runBtn.textContent = state === 'playing' ? '⏸' : '▶';
...
// "all mode": nothing is selected (no token, no node/process) → the node list shows ALL tokens.
SimulationPanel.prototype._allMode = function() {
  const hasSelected = Array.from(this._selectedList.children).some(r => r.style.display !== 'none');
  return !hasSelected && this._nodeIds.length === 0;
};

// Whether a token belongs in the node list right now: every token in all-mode, else those at an
// inspected node.
SimulationPanel.prototype._listsNode = function(node) {
  return this._allMode() || this._nodeIds.indexOf(node) !== -1;
};

// Reconcile the node list to its desired contents (all tokens in all-mode, else tokens at inspected
// nodes) by diff — drop rows no longer wanted, append missing ones. Existing rows are left in place.
SimulationPanel.prototype._syncNodeList = function() {
  const desired = this._allMode()
    ? this._primitives.getTokens()
    : this._primitives.getTokens(t => this._nodeIds.indexOf(t.node) !== -1);
...
// node in all-mode).
SimulationPanel.prototype._onTokenAdded = function(token) {
...
SimulationPanel.prototype._onTokenRemoved = function(token) {
  let changed = false;
...
SimulationPanel.prototype._onTokenUpdated = function(token) {
  for (const ul of [ this._selectedList, this._nodeList ]) {
    const row = findByToken(ul, token);
    if (row) {
...
SimulationPanel.prototype._onTokenMoved = function(e) {
...
SimulationPanel.prototype._onSelectionToggled = function(e) {
  const token = e.token;
  if (token) {
...
SimulationPanel.prototype._onNodeSelectionChanged = function() {
...
SimulationPanel.prototype._refreshRows = function() {
  for (const ul of [ this._selectedList, this._nodeList ]) {
...
// Toggle section visibility from current row counts — display flips only, never a rebuild.
SimulationPanel.prototype._updateVisibility = function() {
  ...
  const allMode = !hasSelected && !hasNodes; // nothing selected → list all tokens
  ...
  // the node section shows tokens at the selected node(s), or — when nothing is selected — all tokens
  this._nodeTitle.textContent = hasNodes ? 'Tokens at selected node(s)' : 'All tokens';
  showEl(this._nodeTitle, (hasNodes || allMode) && hasNodeRows);
  showEl(this._nodeList, (hasNodes || allMode) && hasNodeRows);
  showEl(this._nodeEmpty, hasNodes && !hasNodeRows);
  showEl(this._emptyHint, allMode && !hasNodeRows);
};
...
SimulationPanel.prototype._makeRow = function(token) {
  ...
  info.appendChild(text('div', 'bjs-sim-token-node', ''));
...
SimulationPanel.prototype._updateRow = function(row, token) {
  row._token = token;
  row.dataset.label = token.label;
  row.dataset.node = token.node;
  row.classList.toggle('selected', !!token.selected);

  const sw = row.querySelector('.bjs-sim-swatch');
  if (sw) {
    sw.style.backgroundColor = token.color || '#888';
  }

  const nodeEl = row.querySelector('.bjs-sim-token-node');
  if (nodeEl) {
    nodeEl.textContent = this._displayNode(token.node);
  }

  const badge = row.querySelector('.bjs-sim-badge');
...
SimulationPanel.prototype._onRowClick = function(token) {
  if (this._clickTimer) {
...
SimulationPanel.prototype._onRowDblClick = function(token) {
...
SimulationPanel.prototype._advanceToken = function(token) {
