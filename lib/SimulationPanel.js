...
SimulationPanel.prototype._renderInspector = function() {
  ...
  if (!nodes.length && rootEl && is(rootEl, 'bpmn:Process')) {
    nodes = [ rootEl.id ];
  }
  ...
  nodes.forEach(node => {
    ...
    root.appendChild(text('div', 'bjs-sim-list-title', `Tokens at ${node} (${tokens.length})`));
    if (!tokens.length) {
...
SimulationPanel.prototype._tokenRow = function(token) {
  ...
  row.appendChild(text('span', 'bjs-sim-token-label', token.label));
