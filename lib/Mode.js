...
//
// HACK — the methods are documented, the technique is not. Replacing a service's method reaches around
// whatever else has been layered on it, and a caller is refused in silence, which is why a context pad
// entry could look enabled while doing nothing. The documented way to refuse an operation is a rules
// provider, which diagram-js's own features consult before they offer a gesture, and it is what this
// should become: the guards would then narrow to what a rule cannot express, direct editing and the
// palette. The exceptions above are shaped so that such a move changes nothing a host has written.
