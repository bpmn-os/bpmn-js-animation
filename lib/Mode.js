 ...
 * names. A drag starts when some exception applies to the element the gesture is about, the operation the
 * gesture ends in being checked again when it is issued. The context pad opens only where an exception
...
Mode.prototype._installModelingGuards = function() {
  ...
  // a drag may start on an element some exception is about; what it ends in is checked when it is issued
  ignoreUnlessPermitted(dragging, 'init', dragged);
