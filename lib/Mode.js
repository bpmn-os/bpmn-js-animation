...
Mode.prototype._apply = function() {
  ...
  if (panel && panel.setMode) {
    // panel modes: 'play' (playback), 'simulate', or 'model' (shows only the call-to-action note)
    panel.setMode(mode === 'playback' ? 'play' : mode);
  }
...
Mode.prototype._installModelingGuards = function() {
  ...
  ignoreIfDisabled(contextPad, 'open');        // no editing context pad outside 'model'
