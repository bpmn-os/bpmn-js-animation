 ...
 */
AnimationAPI.prototype.throwIcon = function(node) {
 ...
 * @return {Promise<void>} resolves when the effect ends
 */
AnimationAPI.prototype.catchIcon = function(node) {
...
AnimationAPI.prototype._animateIcon = function(node, direction) {
  const element = this._elementRegistry.get(node);
...
AnimationAPI.prototype._dotHTML = function(token, big) {
  const { position, sequenceFlow, bounce, hidden } = token.state;

  return `
    <div class="bts-token-count waiting${bounce ? ' bts-bounce' : ''}${token.selected ? ' bts-selected' : ''}${hidden ? ' bts-hidden' : ''}${big ? ' bts-on-activity' : ''}"
...
/** Partial merge for setState: position/sequenceFlow stay mutually exclusive, bounce/hidden independent. */
