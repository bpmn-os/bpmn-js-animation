...
// px to drop top-edge sweep tokens when the activity has a boundary event on its top edge, so the
// dots don't sit under the boundary symbol
 ...
 */
export default function AnimationAPI(config, eventBus, canvas, overlays, elementRegistry, outline) {
 ...
 * @return {Token}
 */
AnimationAPI.prototype.createToken = function(node, label, color, state, stackIndices) {
 ...
 */
AnimationAPI.prototype.sendToken = function(transitions) {
 ...
 * @return {Token}
 */
AnimationAPI.prototype.setState = function(node, label, state, selector) {
 ...
 */
AnimationAPI.prototype.glideToState = function(node, label, state, selector, via) {
...
AnimationAPI.prototype._setState = function(node, label, state, selector, onSettled = noop, via) {
  label = String(label);
  ...
  // resting point didn't move (e.g. an animate-only change), or the token is **hidden** (it's
  // invisible — a glide would show a moving graphic; jump instantly so a parked token can be
  // repositioned unseen, then unhidden at the destination).
  ...
  if (this._duration > 0 && moved && !token.state.hidden) {
    ...
    if (movement) {
      ...
      // a glide on a non-front instance (a background instance advancing) must not show its
      // moving dot on top — hide it at the start, like sendToken does (`_syncMovements` then
      // keeps it in sync if the front changes mid-glide)
 ...
 * @param {{ sequenceFlow?: string, stackIndices?: Object }} [selector]
 */
AnimationAPI.prototype.removeToken = function(node, label, selector, gesture) {
  label = String(label);
...
AnimationAPI.prototype._ghostLayer = function() {
...
AnimationAPI.prototype._ghostFor = function(token) {
...
AnimationAPI.prototype._playGhost = function(ghost, effects) {
...
/**
 * **Move** a token to a different node, **preserving its identity** (the same Token object — its color,
 * selection, instance, and tree position are untouched). Re-keys it from `fromNode` to `toNode` and
 * re-renders both — an instantaneous relocation (no flow travel), as a **link event** jumps a token
 * from its throw to the matching catch. `state` merges into the landing rest state (e.g. centered).
 *
 ...
 * @return {Token}
 */
AnimationAPI.prototype.moveToken = function(fromNode, label, toNode, selector, state) {
  this._requireElement(toNode);
  label = String(label);

  const sel = selector || {};
  const key = this._key(fromNode, label, sel.sequenceFlow, sel.stackIndices || {});
  const token = this._head(key);

  if (!token) {
    throw new Error(`moveToken: no token <${label}> at <${fromNode}>`);
  }
  ...
  this._dropToken(key, token);
  this._removeFromNode(token, fromNode);

  token.node = toNode;
  token.state = mergeState(token.state, state || {});

  const newKey = this._key(toNode, label, token.state.sequenceFlow, token.stackIndices);
  this._pushToken(newKey, token);
  this._addToNode(token, toNode);

  this._renderNode(fromNode);
  this._renderNode(toNode);
  return token;
};
 ...
 * @return {Token}
 */
AnimationAPI.prototype.selectToken = function(node, label, selector) {
 ...
 */
AnimationAPI.prototype.setFlowDimmed = function(flowId, on = true) {
 ...
 * @param {boolean} [on=true]
 */
AnimationAPI.prototype.setNodeDimmed = function(nodeId, on = true) {
 ...
 * @return {Token}
 */
AnimationAPI.prototype.deselectToken = function(node, label, selector) {
 ...
 * @return {Token[]}
 */
AnimationAPI.prototype.getSelectedTokens = function() {
 ...
 * @return {string[]}
 */
AnimationAPI.prototype.getSelectedNodes = function() {
 ...
 * @return {Token[]}
 */
AnimationAPI.prototype.getTokens = function(filter) {
 ...
 *
 * @param {string} node
 * @param {string|number} key instance key (a SimulationAPI label, or a numeric index)
 * @return {Promise<void>} resolves when the gesture ends
 */
AnimationAPI.prototype.moveToFront = function(node, key) {
 ...
 */
AnimationAPI.prototype.moveToBack = function(node, key) {
  const order = this._resolveOrder(node);
 ...
 */
AnimationAPI.prototype._renderStackSubtree = function(node, redrawSelf = true) {
 ...
 */
AnimationAPI.prototype._syncMovements = function() {
...
AnimationAPI.prototype.clear = function() {
 ...
 */
AnimationAPI.prototype.setAnimationDuration = function(duration) {
  this._duration = duration;
};

AnimationAPI.prototype.getAnimationDuration = function() {
 ...
 * @return {number}
 */
AnimationAPI.prototype.getMaxVisible = function() {
 ...
 */
AnimationAPI.prototype.throwIcon = function(node, label, selector) {
 ...
 * @return {Promise<void>} resolves when the effect ends
 */
AnimationAPI.prototype.catchIcon = function(node, label, selector) {
 ...
 */
AnimationAPI.prototype.setNodeSelected = function(node, selected = true) {
  this._requireElement(node);
...
AnimationAPI.prototype._stackExtent = function(node) {
 ...
 */
AnimationAPI.prototype._drawNodeOutline = function(node, element, gfx) {
 ...
 */
AnimationAPI.prototype._stackMarkerWidth = function(node) {
 ...
 */
AnimationAPI.prototype.setStacks = function(node, keys, ancestorStackIndices) {
  this._requireElement(node);
...
AnimationAPI.prototype.getStacks = function(node) {
  this._requireElement(node);
 ...
 */
AnimationAPI.prototype._redrawStack = function(node) {
  const element = this._elementRegistry.get(node);
  const size = this._stackSize(node);
 ...
 */
AnimationAPI.prototype._refreshNativeOutline = function(element) {
 ...
 */
AnimationAPI.prototype._drawStackMarker = function(node, element) {
 ...
 * @return {number}
 */
AnimationAPI.prototype._stackSize = function(node) {
 ...
 */
AnimationAPI.prototype._stackGfx = function(element) {
...
AnimationAPI.prototype._isProcessBox = function(element) {
 ...
 */
AnimationAPI.prototype._processBoxContent = function() {
 ...
 * @return {string|null}
 */
AnimationAPI.prototype.getProcessBox = function() {
 ...
 */
AnimationAPI.prototype._ensureProcessBox = function(element) {
  if (this._processBox && this._processBox.id === element.id) {
    return;
  }
...
AnimationAPI.prototype._drawProcessBox = function(element) {
...
AnimationAPI.prototype._removeProcessBox = function() {
  const box = this._processBox;
 ...
 * @return {number}
 */
AnimationAPI.prototype.getCurrentStack = function(node) {
  const order = this._resolveOrder(node);
 ...
 */
AnimationAPI.prototype.getCurrentStacks = function(node) {
  this._requireElement(node);
 ...
 * @return {Promise<void>} resolves when the gesture ends
 */
AnimationAPI.prototype.scrollStack = function(node, direction = 'forward') {
  const element = this._elementRegistry.get(node);
 ...
 * @return {Promise<void>}
 */
AnimationAPI.prototype._animateStackStep = function(node, element, direction, reorder) {
...
AnimationAPI.prototype._cloneVisual = function(visual) {
 ...
 */
AnimationAPI.prototype._cloneNodeVisual = function(element, gfx, withContent) {
 ...
 */
AnimationAPI.prototype._drawTokenDots = function(group, element) {
  const nodes = new Set();
 ...
 */
AnimationAPI.prototype._coRendered = function(de, element) {
  if (de.id === element.id) {
    return true;
  }
 ...
 */
AnimationAPI.prototype._appendTokenDot = function(group, element, tokenElement, token) {
 ...
 */
AnimationAPI.prototype._inlineMarkers = function(group) {
...
AnimationAPI.prototype._clearStackShapes = function(gfx) {
  if (!gfx) {
    return;
  }
...
AnimationAPI.prototype._animateIcon = function(node, label, selector, direction) {
  const element = this._elementRegistry.get(node);
 ...
 * @return {Promise}
 */
AnimationAPI.prototype.playTokenEffect = function(node, label, effect, selector) {
  if (!this._elementRegistry.get(node)) {
    throw new Error(`unknown node <${node}>`);
  }
...
AnimationAPI.prototype._playOnce = function(dot, effect) {
 ...
 */
AnimationAPI.prototype._dotElement = function(token) {
...
AnimationAPI.prototype._key = function(node, label, sequenceFlow, stackIndices) {
...
AnimationAPI.prototype._dropToken = function(key, token) {
  const list = this._tokens.get(key);
  if (!list) {
    return;
  }
...
AnimationAPI.prototype._allTokens = function() {
  const out = [];
 ...
 */
AnimationAPI.prototype._contextKey = function(indices) {
 ...
 */
AnimationAPI.prototype._currentContext = function(node) {
  const ctx = {};
  const element = this._elementRegistry.get(node);
...
AnimationAPI.prototype._resolveOrder = function(node) {
  const orders = this._stackOrder.get(node);
...
AnimationAPI.prototype._visibleTokensAt = function(node) {
...
AnimationAPI.prototype._find = function(node, label) {
  return this._allTokens().filter(token => token.node === node && token.label === label);
};

AnimationAPI.prototype._setTokenSelected = function(node, label, selector, selected) {
  label = String(label);
...
AnimationAPI.prototype._requireElement = function(node) {
  if (!this._elementRegistry.get(node)) {
    throw new Error(`unknown node <${node}>`);
  }
};
 ...
 * node is unknown, or when the viewer has no planes (older bpmn-js). Used by `simulation.replay`.
 *
 * @param {string} node element id
 */
AnimationAPI.prototype.drillTo = function(node) {
  const canvas = this._canvas;
...
AnimationAPI.prototype._shapeOf = function(el) {
 ...
 */
AnimationAPI.prototype._scrollOnDoubleClick = function(event) {
 ...
 */
AnimationAPI.prototype._selectOnClick = function(event) {
...
AnimationAPI.prototype._isDescendant = function(childId, ancestorId) {
...
AnimationAPI.prototype._resolveFlow = function(node, sequenceFlowId) {
 ...
 */
AnimationAPI.prototype._isVisible = function(token) {
  ...
  while (el) {
    el = this._shapeOf(el);
    if (!(onFlow && ownNode) && this._stackSize(el.id) > 1) {
...
AnimationAPI.prototype._planeLayer = function(element) {
  const canvas = this._canvas;
...
AnimationAPI.prototype._settle = function(token) {
  const movement = this._activeAnimations.get(token);
...
AnimationAPI.prototype._addToNode = function(token, node) {
...
AnimationAPI.prototype._removeFromNode = function(token, node) {
  const set = this._nodeTokens.get(node);
...
AnimationAPI.prototype._renderNode = function(node) {
  ...
  for (const tokens of clusters.values()) {
    ...
    domEvent.bind(html, 'click', event => {
      ...
      if (domClasses(el).has('bts-overflow')) {
        ...
        // synthesize `token.dblclick` (the simulator's advance gesture) from two clicks on the
        // same token — a native dblclick can't survive the selection re-render between clicks.
        // (The dot is an HTML overlay, so this never collides with `element.dblclick` on the shape.)
        const dblKey = `${node}|${el.dataset.label}|${el.dataset.sequenceFlow || ''}|${el.dataset.stackIndices || ''}`;
        const now = Date.now();
        const last = this._lastTokenClick;

        if (last && last.key === dblKey && (now - last.time) < DOUBLE_CLICK_MS) {
          this._lastTokenClick = null;
          fireToken('token.dblclick', el, event);
        } else {
          this._lastTokenClick = { key: dblKey, time: now };
        }
      }
 ...
 */
AnimationAPI.prototype._clusterPoint = function(element, state) {
...
// If the activity has any boundary event on its **top edge**, the top-edge sweep positions
// (entry/busy/completion, `top: 0`) would sit under the boundary symbols — so drop those points by
// a fixed `BOUNDARY_VOFFSET`, keeping their `x` (the whole row shifts down together). Points already
// in the lower half (e.g. a center anchor) and activities with no top boundary are left as-is.
AnimationAPI.prototype._avoidBoundaryEvents = function(element, pt) {
  ...
  const topBoundary = attachers.some(b => b.y + b.height / 2 - element.y < half);

  return topBoundary && pt.y < half ? { x: pt.x, y: pt.y + BOUNDARY_VOFFSET } : pt;
};

AnimationAPI.prototype._dotHTML = function(token, big) {
...
AnimationAPI.prototype._markerHTML = function(count, big) {
  return `
 ...
 */
AnimationAPI.prototype._move = function(connection, token, done = noop, duration = this._duration) {
...
AnimationAPI.prototype._stopMovement = function(movement) {
  movement.remove();
  this._movements.delete(movement);
};

AnimationAPI.prototype._movementGroup = function(token) {
