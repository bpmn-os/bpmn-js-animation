...
AnimationAPI.prototype._renderNode = function(node) {
  ...
  for (const tokens of clusters.values()) {
    ...
    // double-click a token dot → `token.dblclick` (the interactive simulator's advance
    // gesture). The dot is an HTML overlay, a distinct target from the SVG shape that
    // `element.dblclick` (stack-scroll) fires on, so the two never collide.
    domEvent.bind(html, 'dblclick', event => {
      const el = domClosest(event.target, '.bts-token-count', true);

      if (!el || domClasses(el).has('bts-overflow')) {
        return;
      }
