 ...
 * @param {{ sequenceFlow?: string, stackIndices?: Object }} [selector]
 */
AnimationAPI.prototype.removeToken = function(node, label, selector) {
  label = String(label);
...
AnimationAPI.prototype.playTokenEffect = function(node, label, effect, selector) {
  ...
  // one-shot gestures are quick feedback (flip on trigger, fade before consume), so they run at a
  // third of a token-move — snappier than a full `animationDuration`; still instant at duration 0
