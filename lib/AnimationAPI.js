...
AnimationAPI.prototype.removeToken = function(node, label, selector, gesture) {
  ...
  // The model identity goes immediately (synchronous); the gesture is purely cosmetic, fire-and-forget.
  const ghost = (gesture && gesture.length) ? this._ghostFor(token) : null;
...
// Play the one-shot effects in sequence on a detached ghost, then remove it (fire-and-forget).
