...
// The layer the process box is drawn into, and where it sits among the layers. diagram-js draws a plane's
// content at index zero and its utility layers above it, so a negative index is below everything, which is
// where a frame belongs.
// what a box borrows from the root it frames, and gives back exactly as it found it
const BOUNDS = [ 'x', 'y', 'width', 'height' ];

const PROCESS_BOX_LAYER = 'processBox';
const PROCESS_BOX_LAYER_INDEX = -1;
 ...
 */
/**
 * The inset of the selection outline of a **stacked** node, which this package sizes itself.
 *
 * It is deliberately a constant of our own rather than diagram-js's inset, which that library does not
 * expose: it was the field `offset` up to diagram-js 15.14 and `_outlineOffset` after, so reading either is
 * reading an implementation detail that has already been renamed once. Read blindly it yields `undefined`
 * against the other version, and `-undefined` is `NaN`, which reaches the SVG as `x="NaN"` and is discarded
 * there without a word, collapsing the outline to nothing at the origin. Five is what diagram-js uses, so
 * the two look alike, and an unstacked node is left to the library entirely.
 */
...
export default function Primitives(config, eventBus, canvas, overlays, elementRegistry, outline) {
  ...
  // keep the moving-token overlay (a layer above the HTML overlays) aligned with pan / zoom, and — on a
  // drill (active-root switch) — re-point its transform and show only the active plane's moving tokens
  eventBus.on([ 'canvas.viewbox.changed', 'root.set' ], () => this._refreshMovementOverlay());

  // the implicit-process box frames one root, and only while that root is the one shown
  eventBus.on('root.set', ({ element }) => this._followRoot(element));
...
Primitives.prototype._ghostFor = function(token) {
  ...
  // The ghost layer is at container scale, outside the zoomed viewport, so the clone would flip-fade at
  // its unzoomed size and appear to grow when the diagram is zoomed out. Wrap it in a div scaled to the
  // current canvas zoom (positioned at the dot's on-screen spot); the flip/fade animate the inner clone.
  const zoom = (this._canvas.zoom && this._canvas.zoom()) || 1;
  const wrap = domify('<div class="bts-token-ghost-wrap"></div>');
  wrap.style.position = 'absolute';
  wrap.style.left = (dRect.left - cRect.left) + 'px';
  wrap.style.top = (dRect.top - cRect.top) + 'px';
  wrap.style.transformOrigin = 'top left';
  wrap.style.transform = 'scale(' + zoom + ')';
  wrap.appendChild(ghost);

  this._ghostLayer().appendChild(wrap);
  return wrap;
};
...
Primitives.prototype._playGhost = function(ghost, effects) {
  const dot = ghost.firstElementChild || ghost; // `ghost` is the zoom-scaled wrapper; animate the clone
...
Primitives.prototype.clear = function() {
  ...
  // drop the moving-token overlay layer (rebuilt lazily on the next move)
  if (this._movementOverlayEl && this._movementOverlayEl.parentNode) {
    this._movementOverlayEl.parentNode.removeChild(this._movementOverlayEl);
  }
  this._movementOverlayEl = null;
  this._movementViewport = null;
  this._movementGroups = null;
...
/**
 * The node's ordered instance keys (front first). In its **current** context by default; pass
 * `ancestorStackIndices` to read a **specific** context's order instead of the current front — the
 * read counterpart of `setStacks`'s context argument, so a stacked child can be added to / removed
 * from the context of the **instance it belongs to** rather than whichever ancestor is on screen.
 * `[]` if unstacked / no such context.
 */
Primitives.prototype.getStacks = function(node, ancestorStackIndices) {
  this._requireElement(node);
  if (ancestorStackIndices === undefined) {
  ...
  return (orders.get(this._contextKey(ancestorStackIndices)) || []).slice();
};
...
Primitives.prototype._refreshNativeOutline = function(element) {
  // `hasBounds` rather than a test for absence: bounds that are present but not finite are the case that
  // reached diagram-js's outline sizing and came out as `x="NaN"`, which a renderer discards in silence.
  if (!this._outline || !hasBounds(element) || is(element, 'bpmn:Process')) {
    return;
  }
...
Primitives.prototype._processBoxContent = function() {
  ...
  // The content is the active layer's own children. The box is not among them, living in a layer of its
  // own, so nothing is filtered out here.
  return Array.from(this._canvas.getActiveLayer().children);
};

/**
 * Follow a drill: the box frames the root it was drawn for, and belongs on screen only while that root is
 * the one shown.
 *
 * The box lives in a layer of its own, which diagram-js neither hides nor shows with a plane, so drilling
 * into a collapsed sub-process would otherwise leave the outer process's frame standing over the plane the
 * reader drilled into. Drilling back out draws it again, since nothing else would: a plane switch moves no
 * token, so no redraw would follow, and the frame would stay missing until one did.
 */
Primitives.prototype._followRoot = function(root) {
  const shown = root && root.id;

  if (this._processBox && this._processBox.id !== shown) {
    this._removeProcessBox();
  }

  const element = shown && this._elementRegistry.get(shown);

  if (element && is(element, 'bpmn:Process') && this._stackSize(shown) >= 1) {
    this._redrawStack(shown);
  }
};
...
Primitives.prototype._ensureProcessBox = function(element) {
  // The box is the process's own shape, and a shape belongs to the plane it is on. A redraw is asked for
  // whenever the stack changes, and that happens while another plane is drilled into: a token advancing
  // into a collapsed sub-process redraws the outer process's stack while the reader is watching the
  // sub-process. Drawing here would put the outer frame over the plane the reader is looking at, the box
  // having a layer of its own that no plane hides. The stack itself is still recorded, and the box is
  // drawn on the way back out.
  const root = this._canvas.getRootElement && this._canvas.getRootElement();

  if (root && root.id !== element.id) {
    return;
  }

  // Only a shape that has bounds can be wrapped. A child without them — a label whose text is not laid
  // out, an element imported without diagram interchange — would make `getBBox` return NaN, and a box
  // drawn from that reaches the SVG as `x="NaN"`, which the renderer discards without telling anyone: the
  // box collapses to nothing at the origin and the tokens resting on it go with it.
  //
  // An artifact is not wrapped. The box stands for the pool a process without one would have been drawn
  // in, and what it frames is what the process executes: a text annotation or a group says something
  // about the diagram rather than taking part in it, and it may be placed anywhere, so wrapping it
  // stretches the frame across empty space to reach a comment.
  const shapes = (element.children || []).filter(wrappable);
  ...
  if (!hasBounds(bounds)) {
    throw new Error(`_ensureProcessBox: "${element.id}" has no bounds to wrap (${JSON.stringify(bounds)})`);
  }

  // A box already around this root and this extent is the box wanted, and is left alone. One around a
  // different extent is not: the content it frames has moved or gained a member since it was drawn, and a
  // box that is only ever drawn once would frame where the content used to be.
  if (this._processBox && this._processBox.id === element.id
      && element.x === bounds.x && element.y === bounds.y
  ...
  // Save + set the root's bounds (bare roots have none). What is saved is which of them the root *had*,
  // not only what they were: diagram-js types an element by the presence of a property rather than by its
  // value (`getType`: `'waypoints' in element` is a connection, `'x' in element` is a shape, and neither is
  // a root), so a bound restored by assigning `undefined` leaves the property in place and leaves the root
  // typed as a shape for good. See `_removeProcessBox`, which is where that matters.
  const savedBounds = {};

  BOUNDS.forEach((key) => {
    if (key in element) {
      savedBounds[key] = element[key];
    }
  });
...
// Used for the implicit-process root, whose bounds we mutate directly.
//
// HACK — undocumented interface. `overlays._getOverlayContainer` and `_updateOverlayContainer` are
// diagram-js internals, and there is no documented way to move a container whose element's bounds changed
// without telling the diagram the element changed. The documented route, firing `element.changed`, has
// diagram-js draw a selection outline for the root and re-evaluate its hit area, which mis-routes a double
// click inside the process box; removing and re-adding every overlay of the element instead would lose the
// ids their owners hold. Both are guarded for, so a diagram-js that drops them leaves the token where it
// was rather than failing. It should be replaced the day diagram-js offers a way to say that an element's
// bounds have moved.
...
// Whether something carries a drawable box. Bounds that are absent or not finite are the one input that
// turns into an `x="NaN"` the renderer throws away in silence, so they are refused rather than drawn.
function hasBounds(element) {
  return [ element.x, element.y, element.width, element.height ].every(Number.isFinite);
}

// Whether the process box wraps a child of the root: a shape rather than a connection, carrying bounds,
// and taking part in the process rather than commenting on it.
function wrappable(element) {
  return !element.waypoints && hasBounds(element) && !is(element, 'bpmn:Artifact');
}
...
Primitives.prototype._drawProcessBox = function(element) {
  ...
  // In a layer of its own, below every plane layer. Drawn into the active layer instead it would be a
  // stranger among the children diagram-js manages there, and diagram-js reorders them: adding any shape
  // to the root moves the box to the end of the layer, which is in front of every node, so the frame comes
  // to cover what it frames. A layer is a child of the viewport, so it still pans and zooms with the
  // diagram, and a negative index keeps it behind the content whichever plane is shown.
  svgAppend(this._canvas.getLayer(PROCESS_BOX_LAYER, PROCESS_BOX_LAYER_INDEX), g);
...
Primitives.prototype._removeProcessBox = function() {
  ...
  // The box is the only shape the implicit process has, so our selection outline was framed on it.
  // Drop that outline now (while the box still exists, so it's removed cleanly) — otherwise the node
  // stays in `_selectedNodes` and a later render repaints the outline at the root's restored,
  // bounds-less / stale coordinates (a stray blue box at a random spot when the box is gone).
  if (this._selectedNodes.has(box.id)) {
    this.setNodeSelected(box.id, false);
  }

  const element = this._elementRegistry.get(box.id);

  if (element) {
    // A bound the root did not have is removed rather than set to `undefined`. diagram-js types an element
    // by which properties it carries, so a root left carrying an `x` of `undefined` is a shape from then
    // on, whatever its value: `Canvas._clear` then sends it to `_removeElement` instead of
    // `removeRootElement`, its plane and its layer are never unwound, and the next diagram is drawn into a
    // canvas whose viewport is no longer in the document — an empty canvas that loading another model does
    // not cure, because the mistyping outlives every clear.
    const saved = box.savedBounds;

    BOUNDS.forEach((key) => {
      if (key in saved) {
        element[key] = saved[key];
      } else {
        delete element[key];
      }
    });
...
// The layer an element's plane is drawn in. A root element carries the name of its own layer, and both
// `findRoot` and `getLayer` are documented, so no plane lookup of diagram-js's own is reached into.
Primitives.prototype._planeLayer = function(element) {
  const canvas = this._canvas;
  const root = canvas.findRoot(element);

  return root && root.layer ? canvas.getLayer(root.layer) : null;
};
...
Primitives.prototype._movementGroup = function(token) {
  const viewport = this._movementOverlay();

  if (!viewport) {
    return null;
  }

  // a group per plane (root): the overlay carries one viewport transform (the active plane's), so a
  // token on a non-active plane would otherwise render at the wrong coordinates (and leak onto the
  // visible plane — e.g. drilling out while a sub-process token is mid-glide). We keep per-plane groups
  // and show only the active one (`_updateMovementPlanes`).
  const rootId = this._movementRootId(token.element);

  if (!this._movementGroups) {
    this._movementGroups = new Map();
  }

  let group = this._movementGroups.get(rootId);

  if (!group || !group.parentNode) {
    group = svgCreate('<g class="bts-animation-tokens" />');
    svgAppendTo(group, viewport);
    this._movementGroups.set(rootId, group);
    this._updateMovementPlanes();
  }

  return group;
};

// The id of the plane (root) a token's element lives on, or '' when planes aren't supported (single plane).
Primitives.prototype._movementRootId = function(element) {
  const canvas = this._canvas;
  if (element && 'findRoot' in canvas) {
    const root = canvas.findRoot(element);
    return root ? root.id : '';
  }
  return '';
...
// The currently active plane's root id, or null when planes aren't supported (then everything shows).
Primitives.prototype._activeRootId = function() {
  const canvas = this._canvas;
  if ('getRootElement' in canvas) {
...
// Show only the active plane's moving-token group; hide the others (their coordinates belong to a plane
// that isn't on screen). No-op for the single-plane fallback (activeId null → all visible).
Primitives.prototype._updateMovementPlanes = function() {
  if (!this._movementGroups) {
    return;
  }
  const activeId = this._activeRootId();
  for (const [ rootId, group ] of this._movementGroups) {
    group.style.display = (activeId == null || rootId === activeId) ? '' : 'none';
  }
};

// Re-point the overlay transform to the active plane and refresh which plane's tokens are shown.
Primitives.prototype._refreshMovementOverlay = function() {
  this._syncMovementOverlay();
  this._updateMovementPlanes();
};

// Moving tokens live in a dedicated SVG overlay **above** diagram-js's HTML overlay container (which
// holds the lint issue markers and the resting-token overlays), so a gliding token paints over the
// markers while they stay visible. The overlay's inner <g> mirrors the canvas viewport transform
// (pan / zoom), so a token positioned in diagram coordinates — the tween's `translate` — lands exactly
// where it would on the canvas. It's the same SVG dot as before; only the layer it paints in changes.
Primitives.prototype._movementOverlay = function() {
  if (this._movementOverlayEl && this._movementOverlayEl.parentNode) {
    return this._movementViewport;
  }
  ...
  const svg = svgCreate('<svg class="bts-animation-overlay"></svg>');
  const viewport = svgCreate('<g class="bts-animation-viewport"></g>');
  svgAppend(svg, viewport);
  container.appendChild(svg);

  this._movementOverlayEl = svg;
  this._movementViewport = viewport;
  this._syncMovementOverlay();

  return viewport;
};

// Keep the moving-token overlay's transform in lockstep with the canvas viewport (pan / zoom / plane).
Primitives.prototype._syncMovementOverlay = function() {
  if (!this._movementViewport) {
    return;
  }
  const src = domQuery('.viewport', this._canvas._svg);
  const transform = src && src.getAttribute('transform');
  if (transform) {
    this._movementViewport.setAttribute('transform', transform);
  } else {
    this._movementViewport.removeAttribute('transform');
  }
};
