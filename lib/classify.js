...
/**
 * Classify a bpmn-js element into the descriptor the Animation branches on —
...
export function classify(element) {
  ...
  if (is(element, 'bpmn:Event')) {
    ...
    if (is(element, 'bpmn:EndEvent')) {
      ...
      // to a matching boundary / event-sub (`errorRef`/`escalationRef` = the referenced id, `''` = catch-all)
      return {
        key: 'event:end', profile: 'event', event: 'end',
        terminate: hasEventDefinition(bo, 'bpmn:TerminateEventDefinition'),
        errorRef: eventDefRef(bo, 'bpmn:ErrorEventDefinition', 'errorRef'),
        escalationRef: eventDefRef(bo, 'bpmn:EscalationEventDefinition', 'escalationRef')
      };
    }
    const throwing = is(element, 'bpmn:IntermediateThrowEvent');
    const link = (bo.eventDefinitions || []).find(d => d.$type === 'bpmn:LinkEventDefinition');
    return {
      key: throwing ? 'event:throw' : 'event:catch', profile: 'event',
      event: throwing ? 'throw' : 'catch',
      link: link ? (link.name || '') : undefined // a link throw jumps to the same-named link catch
    };
  }
  ...
  if (is(element, 'bpmn:Activity')) {
    ...
    if (is(element, 'bpmn:SubProcess')) {
      ...
      return {
        ...
        subProcess: true,
        eventSubProcess,
        adHoc: is(element, 'bpmn:AdHocSubProcess'),
        multiInstance,
        loop,
...
// Whether a flow node's businessObject carries an event definition of the given type.
function hasEventDefinition(bo, type) {
  return (bo.eventDefinitions || []).some(d => d.$type === type);
}

// The id referenced by an error/escalation event definition (`''` when present but unref'd =
// catch-all), or `undefined` when the node has no such definition. Shared by error/escalation events.
function eventDefRef(bo, type, refProp) {
  const def = (bo.eventDefinitions || []).find(d => d.$type === type);
  if (!def) {
    return undefined;
  }
  return (def[refProp] && def[refProp].id) || '';
}
