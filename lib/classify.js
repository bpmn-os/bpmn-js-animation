...
export function classify(element) {
  ...
  if (is(element, 'bpmn:Event')) {
    ...
    if (is(element, 'bpmn:EndEvent')) {
      // a terminate end event ends the whole process instance (kills all its other tokens)
      return { key: 'event:end', profile: 'event', event: 'end', terminate: hasEventDefinition(bo, 'bpmn:TerminateEventDefinition') };
    }
