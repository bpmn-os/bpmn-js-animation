...
- **Loading a diagram over a running one no longer leaves an empty canvas.** The box around a pool-less process hangs its bounds on the root element and gave them back when it was removed, but it gave them back by assignment, and a bound the root never had was assigned `undefined`. diagram-js types an element by which properties it carries rather than by their values — `'x' in element` is a shape, and neither that nor `waypoints` is a root — so a root left carrying an `x` of `undefined` was a shape from then on. `Canvas._clear` sent it to `_removeElement` instead of `removeRootElement`, its plane and its layer were never unwound, and the next diagram was drawn into a canvas whose viewport was no longer in the document. Being a property of the element, the mistyping outlived every clear, so neither loading again nor leaving the mode restored anything. The bounds are now given back exactly as they were found, a bound the root did not have being removed rather than emptied.
- **`reveal` brings a token's plane into view, not only its instance, and advancing a token from the Tokens tab reveals it first.** A token inside a collapsed sub-process rests on a plane of its own, and revealing it moved its stack to the front while leaving the reader on whatever plane they were on. Double-clicking such a row therefore ran the step out of sight, which reads as a gesture that did nothing; the same was true of a token on the outer plane while the reader was drilled in. `reveal` now drills to the token's plane before bringing its instance forward, and the panel's advance reveals before firing `token.dblclick`, exactly as selecting from the panel already did.
- **The box around a pool-less process no longer covers the diagram it frames, and no longer stretches to reach a comment.** It was drawn as the first child of the active layer, behind the flow groups, but that layer holds the children diagram-js manages and reorders: adding any shape to the root moved the box to the end of it, in front of every node, and nothing drew it again because a box around the same root was taken to be the box wanted. It now has a layer of its own below every plane layer, which diagram-js never reorders, and it is drawn again whenever the extent of what it wraps has changed rather than only when the root has. A layer of its own is not a plane, so the box follows the active root itself: drilling into a collapsed sub-process removes it rather than leaving the outer frame standing over the plane drilled into, and drilling back out draws it again with the stack it had. It is likewise never drawn while another plane is shown, which is what a token advancing into a collapsed sub-process used to do: the stack it changes is the outer process's, and the frame appeared over the plane the reader was watching. That one is long-standing rather than new. What it wraps is also what the process executes: an artifact is left out, a text annotation or a group being a comment on the diagram that may sit anywhere, which made the frame reach across empty space to enclose one.
- **The outline of a stacked node no longer depends on an interface diagram-js does not expose.** It was sized from the outline service's inset, carried as `offset` up to diagram-js 15.14 and renamed `_outlineOffset` after, so against a newer diagram-js it read `undefined` and every one of the four attributes came out `NaN` — four values a renderer discards in silence, on every stack redraw, leaving the outline collapsed at the origin. The inset is now this package's own constant, five as diagram-js's is, and an unstacked node is left to the library entirely. The plane layer of an element is likewise taken from `findRoot` and `getLayer`, both documented, rather than from diagram-js's own plane lookup, and what still has no documented alternative — moving an overlay container whose element's bounds were changed, and hiding the palette — is marked as such where it stands.
...
- **A read-only mode refuses modelling through the rules rather than by replacing the modeller's methods.** diagram-js asks its rules before it offers a gesture, so an entry whose operation is refused is no longer drawn, resize handles are not shown, and a drag does not engage — where the former interception left a control that looked enabled and did nothing. It also needs no list of `modeling` methods kept in step with diagram-js. What no rule expresses is still intercepted, which is label editing and the editor actions a keyboard fires, and the Delete key is now let through, since `removeSelection` asks the rules itself and removes only what they permit. Programmatic modelling is no longer refused: a host calling `modeling.moveShape` while a run is on does so on purpose.
